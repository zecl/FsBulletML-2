...
let answer (target: string) (c: JsonElement) =
    match target with
    | "XmlScan" -> XmlScan.describe (c.GetProperty("src").GetString()) (c.GetProperty("cursor").GetInt32())
    | "SxmlScan" -> SxmlScan.describe (c.GetProperty("src").GetString()) (c.GetProperty("cursor").GetInt32())
    | "FsbScan" -> FsbScan.describe (c.GetProperty("src").GetString()) (c.GetProperty("cursor").GetInt32())
    | "FsharpScan" -> FsharpScan.describe (c.GetProperty("src").GetString()) (c.GetProperty("cursor").GetInt32())
    | "SourceKind" -> SourceKind.describe (c.GetProperty("id").GetString())
    | "ShareLink" -> ShareLink.describe (c.GetProperty("fragment").GetString())
    | "Scan" -> Scan.describePosition (c.GetProperty("src").GetString()) (c.GetProperty("cursor").GetInt32())
    | "Semantics" -> Semantics.describe (c.GetProperty("src").GetString())
    | "Outline" -> Outline.describe (c.GetProperty("src").GetString())
    | "OutlineEnclosing" ->
    ...
    | "FsharpBlocks" -> FsharpScan.describeBlocks (c.GetProperty("src").GetString())
    | t ->
