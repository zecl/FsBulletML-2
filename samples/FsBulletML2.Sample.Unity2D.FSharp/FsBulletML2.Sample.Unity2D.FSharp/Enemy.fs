...
type Enemy() =
    ...
    [<DefaultValue>]
    val mutable public bullets: BulletmlInfo list
    ...
    [<System.NonSerialized>]
    [<DefaultValue>]
    val mutable public BulletmlInfo: BulletmlInfo

    /// 撃った弾幕の根。ひと回りしたかを見るのに持ち回る。
    [<System.NonSerialized>]
    [<DefaultValue>]
    val mutable public RootSim: BulletSim

    [<DefaultValue>]
    val mutable public MaxLife: int
    ...
    member this.Start() =
        ...
        indexes |> subscribeUntilDestroy this (fun _ -> this.ApplyPattern())
    ...
    member this.IsFinish() =
        if isNull (box this.RootSim) then
            false
    ...
    member this.Shoot() =
        ...
        if (self.Used) then
            ...
            // （撃つ弾ごとの位置がまだ無い）
            let script =
                FsBulletML2.Runner.load loadRand (loadRank ()) this.BulletmlInfo.Bulletml
