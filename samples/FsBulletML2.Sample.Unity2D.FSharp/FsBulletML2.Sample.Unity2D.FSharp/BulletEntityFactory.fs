...
type BulletEntityFactory private () =
    ...
    static member private MeshOfSprite (sprite: Sprite) (name: string) =
        ...
        if
            not (isNull (box sprite))
            && not (isNull sprite.vertices)
    ...
    static member private ConfigureTransparent(mat: Material) =
        ...
        if mat.HasProperty "_DstBlend" then
            mat.SetFloat("_DstBlend", float32 BlendMode.OneMinusSrcAlpha)
    ...
    static member private MaterialOfSprite (sr: SpriteRenderer) (name: string) =
        ...
        elif mat.HasProperty "_Color" then
    ...
    static member Configure(enemySprite: SpriteRenderer, playerSprite: SpriteRenderer) =
        let enemyMesh =
            BulletEntityFactory.MeshOfSprite
                (if isNull (box enemySprite) then
                     null
                 else
                     enemySprite.sprite)
                "g_bullet_s_ecs"
        ...
        let playerMesh =
            BulletEntityFactory.MeshOfSprite
                (if isNull (box playerSprite) then
                     null
                 else
                     playerSprite.sprite)
                "p_bullet_s_ecs"
    ...
    static member private World() = World.DefaultGameObjectInjectionWorld
    ...
    static member Spawn(kind: BulletKind, x: float32, y: float32, root: bool) : BulletSim =
        ...
        if isNull (box world) then
            ...
            sim.BulletType <-
                (if kind = BulletKind.Player then
                     BulletType.Player
                 else
                     BulletType.Enemy)
            ...
            em.AddComponentData(
                entity,
                LocalTransform.FromPositionRotationScale(float3 (x, y, 0.0f), quaternion.identity, 1.0f)
            )
            |> ignore
    ...
    static member SpawnPlayer(position: Vector3, script: BulletmlScript) =
        let sim =
            BulletEntityFactory.Spawn(BulletKind.Player, position.x, position.y, false)
            ...
            if em.Exists entity then
                if em.HasComponent<BulletTag> entity then
                    ...
                    if kind = BulletKind.Player then
                        ...
                        enemyCount <- max 0 (enemyCount - 1)

                em.DestroyEntity entity

    static member DestroySim(sim: BulletSim) =
        if not (isNull (box sim)) then
            BulletEntityFactory.Destroy sim.Entity
    ...
    static member DestroyAllEnemy() =
        BulletEntityFactory.DestroyAll BulletKind.Enemy

    static member IsOffScreen (x: float32) (y: float32) =
        x < BulletEntityFactory.ScreenMinX
        || x > BulletEntityFactory.ScreenMaxX
        || y < BulletEntityFactory.ScreenMinY
        || y > BulletEntityFactory.ScreenMaxY
