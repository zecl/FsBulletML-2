...
type BulletSim() =
    ...
    member this.SetScript(script: BulletmlScript) =
        ...
        this.Run <-
            if isNull (box script) then
                None
            elif this.IsBullet then
                Some(Runner.newShot this.BulletType script)
            else
                Some(Runner.newRoot this.BulletType script)
    ...
    member this.Init() =
        ...
        this.Run <- this.Run |> Option.map (fun r -> Driver.restart (front :> IFrontEnv) r)
    ...
    member this.Step(spawn: Action<BulletSim, BulletRun>) =
        ...
        | None -> ()
        | Some rn ->
            if isNull (box this.Script) then
                ()
            else
