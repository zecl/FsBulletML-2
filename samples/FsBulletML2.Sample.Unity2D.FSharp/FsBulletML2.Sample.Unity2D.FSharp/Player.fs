...
type Player() =
    ...
    [<DefaultValue>]
    val mutable public bombType: GameObject

    [<DefaultValue>]
    val mutable public speed: float32

    [<DefaultValue>]
    val mutable public isBomb: bool

    [<DefaultValue>]
    val mutable private b2wayLeftBulletTask: BulletmlScript option

    [<DefaultValue>]
    val mutable private b2wayRightBulletTask: BulletmlScript option

    [<DefaultValue>]
    val mutable private hommingTask: BulletmlScript option
    ...
    member this.Awake() =
        ...
        this.b2wayLeftBulletTask <-
            Runner.load loadRand (loadRank ()) FsBulletML2.Bullets.Dsl.PlayerBullet.PlayerBullet.b2wayLeftBullet
            |> Some

        this.b2wayRightBulletTask <-
            Runner.load loadRand (loadRank ()) FsBulletML2.Bullets.Dsl.PlayerBullet.PlayerBullet.b2wayRightBullet
            |> Some

        this.hommingTask <-
            Runner.load loadRand (loadRank ()) FsBulletML2.Bullets.Dsl.PlayerBullet.PlayerBullet.homing
            |> Some
    ...
    member this.Start() =
        ...
        |> subscribeUntilDestroy this (fun _ ->
            if this.isBomb then
                Bomb.GenerateBomb(this.bombType, this.transform.position))
    ...
    member this.HitByEnemyBullet() = damageRp.Value <- damageRp.Value + 1
