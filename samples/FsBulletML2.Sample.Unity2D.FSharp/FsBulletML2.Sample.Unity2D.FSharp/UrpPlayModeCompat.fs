...
type UrpPlayModeCompat private () =
    ...
    static member ConfigureUrpUnlit(mat: Material, texture: Texture, color: Color, transparent: bool, additive: bool) =
        if isNull (box mat) then
            ()
        else

            if mat.HasProperty "_BaseMap" then
                mat.SetTexture("_BaseMap", texture)

            if mat.HasProperty "_MainTex" then
                mat.SetTexture("_MainTex", texture)

            mat.mainTexture <- texture

            if mat.HasProperty "_BaseColor" then
            ...
            if transparent then
                if mat.HasProperty "_Surface" then
                ...
                if mat.HasProperty "_Blend" then
                    mat.SetFloat("_Blend", (if additive then 2.0f else 0.0f))

                if mat.HasProperty "_SrcBlend" then
                    mat.SetFloat("_SrcBlend", float32 BlendMode.SrcAlpha)

                if mat.HasProperty "_DstBlend" then
                    mat.SetFloat("_DstBlend", float32 dst)

                if mat.HasProperty "_ZWrite" then
    ...
    static member private EnsureCameras() =
        let cameras =
            UnityEngine.Object.FindObjectsByType<Camera>(FindObjectsInactive.Include)
        ...
        let main =
            ...
            | null ->
                let tagged =
                    cameras
                    |> Array.tryFind (fun c -> not (isNull (box c)) && c.CompareTag "MainCamera")
        ...
        if isNull (box main) then
            ...
            Debug.Log(
                sprintf
                    "UrpPlayModeCompat cameras: main=%s ortho=%b size=%f stack=%d pipeline=%s"
                    main.name
                    main.orthographic
                    main.orthographicSize
                    mainData.cameraStack.Count
                    (if isNull (box GraphicsSettings.defaultRenderPipeline) then
                         "NULL"
                     else
    ...
    static member private EnsureDirectionalLight() =
        ...
        let hasDirectional =
            lights
            |> Array.exists (fun l -> not (isNull (box l)) && l.``type`` = LightType.Directional)
    ...
    static member private UpgradeSceneRenderers() =
        ...
        if isNull (box unlit) then
            ...
            let renderers =
                UnityEngine.Object.FindObjectsByType<Renderer>(FindObjectsInactive.Include)
                    ...
                    if not (isNull shared) then
                        ...
                        for m in 0 .. shared.Length - 1 do
                            ...
                            if not (isNull (box src)) && not (UrpPlayModeCompat.IsUrpShader src.shader) then
                                ...
                                let shader =
                                    if additive && not (isNull (box particle)) then
                                        particle
                                    else
                                        unlit
                                ...
                                let tex =
                                    if src.HasProperty "_MainTex" then
                                        src.GetTexture "_MainTex"
                                    else
                                        src.mainTexture

                                let color =
                                    if src.HasProperty "_Color" then
                                        src.GetColor "_Color"
                                    else
                                        Color.white
                                // 背景の板だけ不透明。ほかは透過で重ねる
                                let transparent =
                                    not (
                                        r :? MeshRenderer
                                        && r.gameObject.name.IndexOf("background", StringComparison.OrdinalIgnoreCase)
                                           >= 0
                                    )

                                let tex =
                                    if isNull (box tex) then
                                        Texture2D.whiteTexture :> Texture
                                    else
                                        tex
    ...
    static member private BindSpriteTextures() =
        ...
        for sr in sprites do
            UrpPlayModeCompat.BindSprite sr
