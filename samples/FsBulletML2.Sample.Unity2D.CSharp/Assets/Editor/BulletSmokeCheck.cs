...
///
/// <b>較正した。</b> 既存が緑のまま、この門だけが赤くなる変異を 2 つ 当てた。
/// <list type="bullet">
/// <item>BulletSim.BulletType の既定値（BulletType.Enemy）を消す
///       → NullReferenceException。<b>これは実際に踏んだ穴で、この門が見つけた</b>
///       —— F# の判別共用体は参照型なので既定が null になる。コンパイルは通る</item>
...
public static class BulletSmokeCheck
{
    ...
    public static void Run()
    {
        ...
        try
        {
            ...
            var info = FsBulletML2.Bullets.EnemyBullet.Sdmkun.SilverGun.b4D_boss_PENTA;
            failures += Fire(
                info.Name,
                info.Script(FrontEnv.Load()),
                BulletKind.Enemy,
            ...
            failures += Fire(
                "自機の 2way（左）",
                Runner.Load(FrontEnv.Load(), FsBulletML2.Bullets.PlayerBullet.PlayerBullet.b2wayLeftBullet),
                BulletKind.Player,
            ...
            failures += Fire(
                "自機のホーミング",
                Runner.Load(FrontEnv.Load(), FsBulletML2.Bullets.PlayerBullet.PlayerBullet.homing),
                BulletKind.Player,
                FsBulletML2.DTD.BulletType.Player,
                2.4f, -5.0f);
        }
