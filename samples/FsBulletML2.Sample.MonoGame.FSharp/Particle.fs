...
type Particle(texture, pos, speed, angle, scale, shrinkRate, duration, color) as this =
    [<DefaultValue>]
    val mutable private texture: Texture2D

    [<DefaultValue>]
    val mutable private pos: Vector2

    [<DefaultValue>]
    val mutable private direction: Vector2

    [<DefaultValue>]
    val mutable private originPos: Vector2

    [<DefaultValue>]
    val mutable private duration: float32

    [<DefaultValue>]
    val mutable private scale: float32

    [<DefaultValue>]
    val mutable private shrinkRate: float32

    [<DefaultValue>]
    val mutable private speed: float32

    [<DefaultValue>]
    val mutable alive: bool

    [<DefaultValue>]
    val mutable private color: Color
