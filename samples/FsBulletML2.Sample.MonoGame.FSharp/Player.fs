...
type Player () as this =
  ...
  let shoot2WayLeftBullet (player:Player) =
    let ``2wayLeftBullet`` = Runner.load (loadEnv ()) (Bulletml.readXml (playerXml "2wayLeft.xml")) |> Some
  ...
  let shoot2WayRightBullet (player:Player) =
    let ``2wayRightBullet`` = Runner.load (loadEnv ()) (Bulletml.readXml (playerXml "2wayRight.xml")) |> Some
    if player.timer > 0 then
      let bullet = new PlayerBullet()
      Manager.addPlayerBulletPos(bullet, new Vector2(this.pos.X + 10.f, this.pos.Y + 1.f ))
  ...
  let shootHomingBullet (player:Player) = 
    let homingBullet = 
      Runner.load (loadEnv ()) (Bulletml.readXml (playerXml "homing.xml")) |> Some
