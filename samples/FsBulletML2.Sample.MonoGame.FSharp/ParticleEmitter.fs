...
type ParticleEmitter() as this =
    [<DefaultValue>]
    val mutable private particles: List<Particle>

    [<DefaultValue>]
    val mutable private rand: Random

    do
        this.particles <- new List<Particle>()
        this.rand <- new Random()

    member this.Clear() = this.particles.Clear()
    ...
    member this.Emmit(texture, pos, scale, shrinkRate, duration, amount, maxSpeed, colour) =
        for i in [ 0 .. amount - 1 ] do
            ...
            let p =
                new Particle(texture, pos, speed, angle, scale, shrinkRate, duration, colour)
