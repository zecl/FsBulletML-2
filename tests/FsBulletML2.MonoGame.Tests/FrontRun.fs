...
///
///     SetScript が newRoot と newShot をどちらで呼ぶか
///     Motion の往復（フロントの x/y/speed/dir/accel と Motion）
///     Frame.Delta を足す向きと係数
///     Spawn が子へ写す位置・向き・速さと、どの一覧へ入れるか
///     Vanished / Retired を Used に落とすところ
///     Finished のコマで走らせ直すこと
///     画面の外へ出た弾を落とすこと
///
/// **変異を並べて数えたら、Front の門 9 通り のうち 4 通り が緑で通った。**
/// そのうち 3 つ は Front と Core に門を足して閉じたが、
/// **フロントそのものを回す門はどこにも無かった**ので、ここで足す。
///
/// 較正は `変異を入れると割れる` の 3 本（この下）。
[<TestFixture>]
[<NonParallelizable>]
type FrontRun() =
