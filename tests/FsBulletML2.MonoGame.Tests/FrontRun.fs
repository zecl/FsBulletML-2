  ...
  <action label="top">
    ...
    let snapshot (i: int) =
        ...
        sprintf
            "f%02d  E[%s]  EB[%s]  PB[%s]"
            i
            (one "e" Manager.enemies)
            (one "b" Manager.enemyBullets)
            (one "p" Manager.playerBullets)
    ...
    member _.``12 コマ のあいだに、撃って・動いて・消して・走らせ直している``() =
        ...
        let births =
            System.Text.RegularExpressions.Regex
                .Matches(trace, System.Text.RegularExpressions.Regex.Escape(FrontRunGolden.Birth))
                .Count
        ...
        let counts =
            lines
            |> Array.map (fun l -> System.Text.RegularExpressions.Regex.Matches(l, @"\bb\d+ x=").Count)

        let shrank =
            Seq.init (counts.Length - 1) id
            |> Seq.exists (fun i -> counts.[i + 1] < counts.[i])
