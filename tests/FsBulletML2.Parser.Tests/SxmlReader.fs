...
type SxmlReader() =
    ...
    let reads (src: string) =
        (SourceReader.sxml.Apply ignore src).IsNone
        ...
        let split =
            ...
            |> Array.choose (fun file ->
                ...
                if mine = theirs then
                    ...
                    Some(sprintf "%s: こちら %b / Parser %b" (Path.GetFileName file) mine theirs))
            |> Array.toList
            ...
            |> Array.choose (fun file ->
                ...
                if not (File.Exists asXml) then
                    ...
                    if a = b then
                        ...
                        Some(sprintf "%s: sxml %b / xml %b" rel a b))
            |> Array.toList
    ...
    member _.``構文が壊れていれば 位置が出る``() =
        ...
        let cases =
            [ "(bulletml (action (fire", 1; "(bulletml\n(action\n(fire \"\n)\n)", 3 ]
    ...
    member _.``桁が動く``() =
        ...
        columnOf "(bulletml (action (fire"
        |> should not' (equal (columnOf "(bulletml (fire"))
    ...
    member _.``拡張子は 表記ごとに違い、id から導けない``() =
        ...
        // 導くと「開いたのに読めない」になる。
        SourceKind.all
        |> List.map (fun k -> k.FileExtension)
        ...
        SourceKind.FSharpDsl.FileExtension
        |> should not' (equal ("." + SourceKind.FSharpDsl.Id))
