...
type QuickFix() =
    ...
    let renames lang marked =
        at lang marked |> List.filter (fun f -> f.EndColumn > f.Column)

    /// 定義を作るほうだけ。幅 0 が挿し込みの印
    let creates lang marked =
        at lang marked |> List.filter (fun f -> f.EndColumn = f.Column)

    let titles lang marked =
        renames lang marked |> List.map (fun f -> f.Title) |> List.sort

    /// 直し方を当てた本文。挿し込みは行と桁で入るので、
    ...
    member _.``作った定義を当てた本文が 読める``() =
        ...
        for (kind, lang: ISourceLanguage, src) in cases do
            ...
            match
                lang.Fixes src (src.IndexOf "\"loop\"" + 2)
    ...
    member _.``F# の CE でも作る（v1.9）``() =
        // v1.6 まで空だった。カーソルが名前の上に無ければ空なのは同じ
    ...
    member _.``sxml でも同じように出る``() =
        titles sxml "(bulletml (action (@ (label \"top\"))) (actionRef (@ (label \"t|p\"))))"
        |> should equal [ "tp を top に直す" ]
        ...
        for file in corpus.Value do
            ...
            for (from, till, original, typo) in broken do
                ...
                if not (List.contains original found) then
                    bad.Add(
                        sprintf "%s: %s -> %s の直し方に %s が無い（%A）" (Path.GetFileName file) original typo original found
                    )
    ...
    member _.``壊していない本文には 1 つ も出さない``() =
        ...
        let noisy =
            ...
            |> Array.choose (fun file ->
                ...
                if found.IsEmpty || (References.missing XmlScan.tags src).Length > 0 then
                    None
                else
                    Some(sprintf "%s: %s" (Path.GetFileName file) found.Head.Title))
            |> Array.toList
