...
module Test =
    ...
    let resolveTestPath (p: string) =
        ...
        let fromOutput =
            Path.GetFullPath(
                Path.Combine(AppContext.BaseDirectory, "TestData", relative.Replace('/', Path.DirectorySeparatorChar))
            )

        if File.Exists(fromOutput) then
            fromOutput
        else
            Path.GetFullPath(
                Path.Combine(__SOURCE_DIRECTORY__, "..", "TestData", relative.Replace('/', Path.DirectorySeparatorChar))
            )
    ...
    let throws (exceptionType: System.Type) (list: TestCaseData list) =
        ...
        | x :: rest ->
            (x.SetProperty("ExpectedException", exceptionType.AssemblyQualifiedName))
            :: rest
