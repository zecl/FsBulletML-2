...
type Frames() =
    ...
    static let shapes = [ "xml", Xml.shape; "sxml", Sxml.shape; "fsb", Fsb.shape ]
    ...
    member _.``3 表記 が同じ木になる``() =
        for snippet in Frames.all do
            let trees =
                shapes
                |> List.map (fun (name, shape) -> name, readWith shape.Kind (fill (shape.WriteFrame(wrap snippet))))
    ...
    member _.``host が焼いた JSON に雛形が載っている``() =
        ...
        for f in arr do
            ...
            f.GetProperty("frame").GetProperty("element").GetString()
            |> should not' (equal "")
