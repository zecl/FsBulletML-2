...
module ``文字列からのパース`` =
    ...
    let ``文字列からのパース`` (source) = parseFromString source
...
module ``XMLファイル:BulletML DTDに基づくパース`` =
        ...
        withExpectedException (fun () ->
            ...
            | Some bulletml ->
                let x =
                    bulletml
                    |> (fun x ->
                        x.ToIndentedXmlStringForTest(encodingAndDoctype = EncodingAndDoctype.Exist),
                        x.ToXmlStringForTest(EncodingAndDoctype.Exist))

                x
                |> should
                    equal
                    (xml.ToIndentedXmlString(encodingAndDoctype = EncodingAndDoctype.Exist),
                     xml.ToXmlString(EncodingAndDoctype.Exist))
    ...
    let ``vertical-要素`` (xmlFile) = parse xmlFile


    [<Test>]
    [<TestCaseSource "VerticalAttributesCase">]
    let ``vertical-属性`` (xmlFile) = parse xmlFile
    ...
    let ``fireRef-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "FireRefAttributesCase">]
    let ``fireRef-属性`` (xmlFile) = parse xmlFile

    // <!ELEMENT changeDirection (direction, term)>
    let ChangeDirectionElementsCase =
        // support
        testCase (
            singleParam @"..\..\..\TestData\xml\changeDirection\elements\success\changeDirection-direction-exist.xml"
        )
    ...
    let ChangeDirectionAttributesCase =
        // not support
        testCase (
            singleParam @"..\..\..\TestData\xml\changeDirection\attributes\failure\changeDirection-attribute-exist.xml"
        )
        &! (typeof<BulletmlDTDViolationException>)

    [<Test>]
    [<TestCaseSource "ChangeDirectionElementsCase">]
    let ``changeDirection-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "ChangeDirectionAttributesCase">]
    let ``changeDirection-属性`` (xmlFile) = parse xmlFile
    ...
    let ``bulletml-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "BulletmlAttributesCase">]
    let ``bulletml-属性`` (xmlFile) = parse xmlFile
    ...
    let ``param-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "ParamAttributesCase">]
    let ``param-属性`` (xmlFile) = parse xmlFile
    ...
    let ``actionRef-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "ActionRefAttributesCase">]
    let ``actionRef-属性`` (xmlFile) = parse xmlFile
    ...
    let ``repeat-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "RepeatAttributesCase">]
    let ``repeat-属性`` (xmlFile) = parse xmlFile
    ...
    let ``accel-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "AccelAttributesCase">]
    let ``accel-属性`` (xmlFile) = parse xmlFile
    ...
    let ``times-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "TimesAttributesCase">]
    let ``times-属性`` (xmlFile) = parse xmlFile
    ...
    let ``term-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "TermAttributesCase">]
    let ``term-属性`` (xmlFile) = parse xmlFile
    ...
    let ``wait-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "WaitAttributesCase">]
    let ``wait-属性`` (xmlFile) = parse xmlFile
    ...
    let ``action-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "ActionAttributesCase">]
    let ``action-属性`` (xmlFile) = parse xmlFile
    ...
    let ``vanish-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "VanishAttributesCase">]
    let ``vanish-属性`` (xmlFile) = parse xmlFile
    ...
    let ``speed-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "SpeedAttributesCase">]
    let ``speed-属性`` (xmlFile) = parse xmlFile
    ...
    let ``horizontal-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "HorizontalAttributesCase">]
    let ``horizontal-属性`` (xmlFile) = parse xmlFile
    ...
    let ``bullet-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "BulletAttributesCase">]
    let ``bullet-属性`` (xmlFile) = parse xmlFile
    ...
    let DirectionAttributesCase =
        ...
        &> (singleParam
                @"..\..\..\TestData\xml\direction\attributes\success\direction-changeDirection-type-absolute.xml")
        &> (singleParam
                @"..\..\..\TestData\xml\direction\attributes\success\direction-changeDirection-type-relative.xml")
        &> (singleParam
                @"..\..\..\TestData\xml\direction\attributes\success\direction-changeDirection-type-sequence.xml")
    ...
    let ``direction-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "DirectionAttributesCase">]
    let ``direction-属性`` (xmlFile) = parse xmlFile
    ...
    let ``changeSpeed-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "ChangeSpeedAttributesCase">]
    let ``changeSpeed-属性`` (xmlFile) = parse xmlFile
    ...
    let ``fire-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "FireAttributesCase">]
    let ``fire-属性`` (xmlFile) = parse xmlFile
    ...
    let ``bulletRef-要素`` (xmlFile) = parse xmlFile

    [<Test>]
    [<TestCaseSource "BulletRefAttributesCase">]
    let ``bulletRef-属性`` (xmlFile) = parse xmlFile
    ...
    let ``非決定値 rand,rank`` (xmlFile) = parse xmlFile
