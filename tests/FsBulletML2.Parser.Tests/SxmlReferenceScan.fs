...
type SxmlReferenceScan() =
  ...
  let explain src = References.explain SxmlScan.tags (read src) src
  ...
  let refAttr = Regex("\\((actionRef|fireRef|bulletRef)\\s*\\(@\\s*\\(label\\s*\"([^\"]*)\"")

  // --- 挙げる / 挙げない ----------------------------------------------------

  [<Test>]
  member _.``定義が在れば挙げない``() =
    missing "(bulletml (action (@ (label \"a\")) (wait \"1\")) (action (@ (label \"top\")) (actionRef (@ (label \"a\")))))"
    |> should be Empty

  [<Test>]
  member _.``無い参照を 2 つ とも挙げる``() =
    let found =
      missing "(bulletml (action (@ (label \"top\")) (actionRef (@ (label \"a\"))) (actionRef (@ (label \"b\")))))"
  ...
  member _.``名前の無い括弧は数えない``() =
    ...
    SxmlScan.tags "(bulletml (action ( )))" |> List.map (fun t -> t.TagName)
