...
type SxmlReferenceScan() =
    ...
    let missing = References.missing SxmlScan.tags

    let rand () = 0.5f
    let rank = 0.5f
    let build (b: Bulletml) = Runner.load rand rank b |> ignore
    let read src = SourceReader.sxml.Apply build src
    let coreFails src = (read src).IsSome

    /// 本番と同じ道。`Main.fs` の `ApplySource` もこれを通る
    let explain src =
        References.explain SxmlScan.tags (read src) src
    ...
    let corpus =
        lazy
            ...
            if Directory.Exists root then
                Directory.EnumerateFiles(root, "*.sxml", SearchOption.AllDirectories)
                |> Seq.toArray
            else
                [||]
    ...
    let refAttr =
        Regex("\\((actionRef|fireRef|bulletRef)\\s*\\(@\\s*\\(label\\s*\"([^\"]*)\"")
    ...
    member _.``定義が在れば挙げない``() =
        missing
            "(bulletml (action (@ (label \"a\")) (wait \"1\")) (action (@ (label \"top\")) (actionRef (@ (label \"a\")))))"
        |> should be Empty
    ...
    member _.``無い参照を 2 つ とも挙げる``() =
        let found =
            missing
                "(bulletml (action (@ (label \"top\")) (actionRef (@ (label \"a\"))) (actionRef (@ (label \"b\")))))"
    ...
    member _.``fire と bullet も見る``() =
        let found =
            missing (
                "(bulletml (action (@ (label \"top\")) (fireRef (@ (label \"f\")))"
                + " (fire (bulletRef (@ (label \"b\"))))))"
            )
    ...
    member _.``位置が label の値そのものを指す``() =
        // 3 行目 の `(actionRef (@ (label "a")))`。`a` は 23 桁目、閉じ引用符が 24
        let src =
            "(bulletml\n(action (@ (label \"top\"))\n(actionRef (@ (label \"a\")))\n)\n)"
        ...
        | [ f ] ->
            ...
            f.Column |> should equal 23
            f.EndColumn |> should equal 24
    ...
    [<Test>]
    member _.``本文の文字列の中は数えない``() =
        // 属性ブロックの外の `"…"` は本文。 数えると、式の中に書いた字が
        // label に見える
        missing "(bulletml (action (@ (label \"top\")) (wait \"(actionRef (@ (label a)))\")))"
        |> should be Empty

    [<Test>]
    member _.``名前の無い括弧は数えない``() =
        // XML 側の「宣言と閉じ札を積まない」に当たる。名前が空の要素を混ぜない ——
        // 混ぜても候補も hover も変わらないので、当たるのはここだけ
        SxmlScan.tags "(bulletml (action ( )))"
