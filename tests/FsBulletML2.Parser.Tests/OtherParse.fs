...
module ``XML, SXML, FSBファイルパース結果比較`` =
    ...
    let ``accel-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "AccelAttributesCase">]
    let ``accel-属性`` (format) = parse format
    ...
    let ``action-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "ActionAttributesCase">]
    let ``action-属性`` (format) = parse format
    ...
    let ``actionRef-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "ActionRefAttributesCase">]
    let ``actionRef-属性`` (format) = parse format
    ...
    let ``bullet-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "BulletAttributesCase">]
    let ``bullet-属性`` (format) = parse format
    ...
    let ``bulletml-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "BulletmlAttributesCase">]
    let ``bulletml-属性`` (format) = parse format
    ...
    let ``bulletRef-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "BulletRefAttributesCase">]
    let ``bulletRef-属性`` (format) = parse format

    // <!ELEMENT changeDirection (direction, term)>
    let ChangeDirectionElementsCase =
        // support
        testCase (
            singleParam @"..\..\..\TestData\{0}\changeDirection\elements\success\changeDirection-direction-exist.{0}"
        )
    ...
    let ChangeDirectionAttributesCase =
        // not support
        testCase (
            singleParam @"..\..\..\TestData\{0}\changeDirection\attributes\failure\changeDirection-attribute-exist.{0}"
        )

    [<Test>]
    [<TestCaseSource "ChangeDirectionElementsCase">]
    let ``changeDirection-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "ChangeDirectionAttributesCase">]
    let ``changeDirection-属性`` (format) = parse format
    ...
    let ``changeSpeed-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "ChangeSpeedAttributesCase">]
    let ``changeSpeed-属性`` (format) = parse format
    ...
    let DirectionAttributesCase =
        ...
        &> (singleParam
                @"..\..\..\TestData\{0}\direction\attributes\success\direction-changeDirection-type-absolute.{0}")
        &> (singleParam
                @"..\..\..\TestData\{0}\direction\attributes\success\direction-changeDirection-type-relative.{0}")
        &> (singleParam
                @"..\..\..\TestData\{0}\direction\attributes\success\direction-changeDirection-type-sequence.{0}")
    ...
    let ``direction-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "DirectionAttributesCase">]
    let ``direction-属性`` (format) = parse format
    ...
    let ``fire-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "FireAttributesCase">]
    let ``fire-属性`` (format) = parse format
    ...
    let ``fireRef-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "FireRefAttributesCase">]
    let ``fireRef-属性`` (format) = parse format
    ...
    let ``horizontal-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "HorizontalAttributesCase">]
    let ``horizontal-属性`` (format) = parse format
    ...
    let ``param-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "ParamAttributesCase">]
    let ``param-属性`` (format) = parse format
    ...
    let ``repeat-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "RepeatAttributesCase">]
    let ``repeat-属性`` (format) = parse format
    ...
    let ``speed-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "SpeedAttributesCase">]
    let ``speed-属性`` (format) = parse format
    ...
    let ``term-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "TermAttributesCase">]
    let ``term-属性`` (format) = parse format
    ...
    let ``times-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "TimesAttributesCase">]
    let ``times-属性`` (format) = parse format
    ...
    let ``vanish-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "VanishAttributesCase">]
    let ``vanish-属性`` (format) = parse format
    ...
    let ``vertical-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "VerticalAttributesCase">]
    let ``vertical-属性`` (format) = parse format
    ...
    let ``wait-要素`` (format) = parse format

    [<Test>]
    [<TestCaseSource "WaitAttributesCase">]
    let ``wait-属性`` (format) = parse format
    ...
    let ``その他-サンプル等`` (format) = parse format
