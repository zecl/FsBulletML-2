...
type Outline() =
            ...
            |> List.choose (fun info ->
                ...
                | Some src ->
                    ...
                    for t in ts do
                        if t.Closing then
                            if stack.Count > 0 then
                                closedAt.[stack.Pop().NameStart] <- lineOf t
                        elif t.SelfClosing then
                            closedAt.[t.NameStart] <- lineOf t
                        else
                            stack.Push t
                    ...
                    let wrong =
                        ...
                        |> List.filter (fun (t, n) ->
                            let e =
                                if closedAt.ContainsKey t.NameStart then
                                    closedAt.[t.NameStart]
                                else
                                    last

                            e <> n.EndLine)

                    if List.isEmpty wrong then
                        None
                    else
                        Some(info.Name, List.length wrong))
                ...
                |> List.choose (fun (kind, tags) ->
                    ...
                    | Some src ->
                        ...
                        if List.isEmpty bad then
                            ...
                            Some(info.Name, kind, List.length bad)))
    ...
    member _.``閉じ札は節にならない``() =
        let opens =
            XmlScan.tags sample |> List.filter (fun t -> not t.Closing) |> List.length

        xmlNodes sample |> List.length |> should equal opens

    [<Test>]
    member _.``空の本文``() = xmlNodes "" |> should be Empty
    ...
    member _.``囲みは内から外へ``() =
        ...
        |> should equal [ "speed"; "fire"; "action"; "bulletml" ]
    ...
    member _.``節の先頭の行では、その節も囲みに入る``() =
        ...
        ns
        |> List.map (fun n -> n.Name)
        |> should equal [ "fire"; "action"; "bulletml" ]
    ...
    member _.``兄弟は混ざらない``() =
        ...
        ns
        |> List.map (fun n -> n.Name)
        |> should equal [ "speed"; "bullet"; "bulletml" ]
