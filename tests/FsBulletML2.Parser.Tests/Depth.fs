...
type Depth() =
    ...
    static let fsharpTags (src: string) =
        ...
        let labels =
            v.CeLabels
            |> List.map (fun c -> c.Name, c.Element, c.LabelArg, c.Fixed)
            |> List.toArray
    ...
    static let labeledOf (tags: string -> TagHit list) (src: string) =
        tags src
        |> List.filter (fun t -> not t.Closing && t.Attrs |> List.exists (fun a -> a.AttrName = "label"))
        |> List.map (fun t -> t.TagName, t.Depth)
            ...
            |> List.choose (fun info ->
                let per =
                    ...
                    |> List.map (fun (label, kind, tags) ->
                        label,
                        (writeAs kind info.Bulletml
                         |> Option.map (shapeOf tags)
                         |> Option.defaultValue []))
    ...
    member _.``fsb は字下げの幅を決め打たない``() =
        ...
        let shape (ts: TagHit list) =
            ts |> List.map (fun t -> t.TagName, t.Depth)
