...
type NodeSpansTests() =
    ...
    static let firesOf (src: string) (tagsOf: string -> TagHit list) =
        ...
        let open_ =
            tags |> List.filter (fun t -> not t.Closing && List.contains t.TagName names)

        List.zip open_ spans
        |> List.filter (fun (t, _) -> t.TagName = "fire")
        |> List.map snd
            ...
            for info in catalog do
                ...
                | Some src ->
                    for sp in Scan.nodeSpans src (tagsOf src) names do
                        ...
                        Assert.That(lineOf src sp.OpenStop, Is.EqualTo(lineOf src sp.OpenStart), name + ": 開き札が行をまたいだ")
            // 0 件 を緑にしない
            ...
            for info in catalog do
                ...
                | Some src ->
                    for sp in firesOf src tagsOf do
                        ...
                        if sp.OpenStop - sp.OpenStart > sp.NameStop - sp.NameStart then
                            wider <- wider + 1
    ...
    member _.``閉じ札の対は xml だけで組める``() =
        ...
        let paired (kind: SourceKind) (tagsOf: string -> TagHit list) =
            ...
            for info in catalog do
                ...
                | Some src ->
                    for sp in firesOf src tagsOf do
                        if sp.CloseStart = sp.OpenStart && sp.CloseStop = sp.OpenStop then
                            same <- same + 1
                        else
                            apart <- apart + 1
    ...
    member _.``1 行 に詰めて書いても、開き札で止まる``() =
        ...
        let src =
            "<bulletml><action label=\"top\"><fire><direction>0</direction><speed>2</speed><bullet/></fire><wait>1</wait></action></bulletml>"

        let spans = Scan.nodeSpans src (XmlScan.tags src) names

        let tags =
            XmlScan.tags src
            |> List.filter (fun t -> not t.Closing && List.contains t.TagName names)
        ...
        src.Substring(fire.OpenStart, fire.OpenStop - fire.OpenStart)
        |> should equal "<fire>"

        src.Substring(fire.CloseStart, fire.CloseStop - fire.CloseStart)
        |> should equal "</fire>"
        // 自己閉じの札は対を持たない（`<bullet/>`）—— 開き札と同じ範囲
        let bullet = byName |> List.find (fun (n, _) -> n = "bullet") |> snd

        src.Substring(bullet.OpenStart, bullet.OpenStop - bullet.OpenStart)
        |> should equal "<bullet/>"
    ...
    member _.``閉じ忘れが在っても、名前が合う札に対を組む``() =
        ...
        let src =
            "<bulletml><action label=\"top\"><fire><bullet></fire><wait>1</wait></action></bulletml>"
        ...
        src.Substring(action.CloseStart, action.CloseStop - action.CloseStart)
        |> should equal "</action>"
