...
type Semantics() =
    ...
    static let findingsOf (src: string) =
        Semantics.findings pairs top (XmlScan.tags src)

    /// 同梱 176 本 を XML の字にしたもの。
    /// 定数を畳まない側で焼く（`Main.fs` の `InitialSource` と同じ）
    static let corpus =
        catalog |> List.map (fun i -> i.Name, BulletmlWriter.toIndentedXml 4 i.Bulletml)
...
<bulletml xmlns="http://www.asahi-net.or.jp/~cs8k-cyu/bulletml">
  ...
  <action label="x"><wait>2</wait></action>
        ...
        let dup =
            findingsOf src |> List.filter (fun f -> f.Kind = Semantics.DuplicateDefinition)
        ...
        [
            for (name, text) in corpus do
                for f in findingsOf text do
                    if f.Kind = Semantics.DuplicateDefinition then
                        yield name, f.Name
        ]
        |> should be Empty
...
<bulletml xmlns="http://www.asahi-net.or.jp/~cs8k-cyu/bulletml">
  ...
  <bullet label="b2"><speed>1</speed></bullet>
    ...
    member _.``同梱に入口の無い本は 1 本 も無い``() =
        ...
        corpus
        |> List.filter (fun (_, src) -> findingsOf src |> List.exists (fun f -> f.Kind = Semantics.NoEntryPoint))
        |> List.map fst
        |> should be Empty
