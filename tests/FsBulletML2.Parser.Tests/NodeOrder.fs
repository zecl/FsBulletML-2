...
type NodeOrderTests() =
    ...
    member _.``並びに 2 度 出るのは vanish と bullet だけ``() =
        ...
        for kv in ceNames do
            Assert.That([ "vanish"; "bullet" ], Does.Contain kv.Key, "CE の木に " + kv.Key + " の重なりが出た")
    ...
    member _.``XML から読み直すと重なるのは vanish だけ``() =
        ...
        for kv in names do
            Assert.That(kv.Key, Is.EqualTo "vanish", "字から読んだ木に " + kv.Key + " の重なりが出た")
