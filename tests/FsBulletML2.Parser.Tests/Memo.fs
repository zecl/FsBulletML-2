...
<bulletml xmlns="http://www.asahi-net.or.jp/~cs8k-cyu/bulletml">
  ...
  <action label="shot">
    ...
    static let alternates (of_: SourceLanguage.ISourceLanguage -> string -> 'T) =
        ...
        let want =
            [ of_ (fresh ()) a; of_ (fresh ()) b; of_ (fresh ()) a; of_ (fresh ()) b ]
    ...
    member _.``CE も本文を取り違えない``() =
        ...
        | [ x; y ] ->
            ...
            let of_ (l: SourceLanguage.ISourceLanguage) s =
                l.Lenses s |> List.map (fun n -> n.Name, n.Title)
...
type LineColumns() =
    ...
    member _.``表で引いても、頭から走っても同じ``() =
        ...
        for i in 0 .. src.Length do
            if lookup i <> Scan.lineColumn src i then
                bad <- bad + 1
    ...
    member _.``昇順が崩れても答えは変わらない``() =
        ...
        Scan.lineColumnsAscending src offs
        |> should equal (offs |> List.map (Scan.lineColumn src))
