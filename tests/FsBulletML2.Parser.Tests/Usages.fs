...
type Usages() =
    ...
    member _.``参照が無い定義でも 自分 1 つ は返る``() =
        ...
        at xml "<bulletml><action label=\"t|op\"/></bulletml>"
        |> List.length
        |> should equal 1
    ...
    member _.``別の名前は混ざらない``() =
        let src =
            "<bulletml><action label=\"a\"/><action label=\"b\"/><actionRef label=\"a\"/></bulletml>"
    ...
    member _.``label でない属性値では 引かない``() =
        ...
        at xml "<bulletml><direction type=\"a|im\">0</direction></bulletml>"
        |> should be Empty
    ...
    member _.``定義側と参照側を 1 欄 で分ける``() =
        ...
        found
        |> List.filter (fun u -> not u.IsDefinition)
        |> List.length
        |> should equal 1
        // 定義のほうが手前に在る。並びは本文の順
            ...
            let others =
                ...
                |> List.choose (fun (ext, lang, tags) ->
                    ...
                    if File.Exists f then
                        Some(ext, profile lang tags (File.ReadAllText f))
                    else
                        None)
