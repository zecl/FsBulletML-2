...
type AttributeDefaults() =
    ...
    static let accelIn (axis: string) (attr: string) =
        sprintf
            "<action label=\"top\"><fire><speed>1</speed><bullet><action><accel><%s%s>2</%s><term>4</term></accel><wait>8</wait></action></bullet></fire><wait>9</wait></action>"
            axis
            attr
            axis
    ...
    static let templates: ((string * string) * (string -> string)) list =
        [
            ...
            (fun attr ->
                sprintf
                    "<bulletml%s xmlns=\"%s\"><action label=\"top\"><fire><bullet/></fire><wait>2</wait></action></bulletml>"
                    attr
                    Ns)
            ...
            (fun attr ->
                wrap (
                    sprintf
                        "<action label=\"top\"><fire><direction%s>30</direction><speed>2</speed><bullet/></fire><wait>3</wait></action>"
                        attr
                ))
            ...
            (fun attr ->
                wrap (
                    sprintf
                        "<action label=\"top\"><fire><speed>3</speed><bullet/></fire><fire><speed%s>2</speed><bullet/></fire><wait>3</wait></action>"
                        attr
                ))

            ("horizontal", "type"), (fun attr -> wrap (accelIn "horizontal" attr))

            ("vertical", "type"), (fun attr -> wrap (accelIn "vertical" attr))
        ]
        ...
        | Some bulletml ->
            ...
            for frame in 1..8 do
                ...
                |> List.iteri (fun i run ->
                    ...
                    sb.Append(sprintf "%d/%d=%.4f,%.4f;" frame i (float f.Delta.X) (float f.Delta.Y))
                    |> ignore

                    next.Add f.Run

                    for c in f.Spawned do
                        next.Add c)
        ...
        let bad =
            ...
            |> Array.choose (fun (el, a) ->
                if a.Defaults.Length <> 1 then
                    ...
                    Some(sprintf "%s/@%s の既定 %s が並び %s に無い" el a.Name a.Defaults.[0] (String.concat "|" a.Values))
                else
                    None)
            |> Array.toList
            ...
            [
                for (el, a) in enumerated do
                    ...
                    | Some(_, build) when a.Defaults.Length = 1 ->
                        ...
                        if not differs then
                            yield sprintf "%s/@%s: %s のどれを書いても走りが変わらない" el a.Name (String.concat "|" others)
                    | _ -> ()
            ]
