...
/// 参照の欠けを数える側の目盛り、fsb の側。 `ReferenceScan` / `SxmlReferenceScan` と対。
/// 数える中身は `References` に 1 本。ここで赤くなるのは `FsbScan`。共通が壊れれば XML と sxml も赤くなる。
[<TestFixture>]
type FsbReferenceScan() =
    ...
    let explain src =
        References.explain FsbScan.tags (read src) src
    ...
    let corpus =
        lazy
            ...
            if Directory.Exists root then
                Directory.EnumerateFiles(root, "*.fsb", SearchOption.AllDirectories)
                |> Seq.toArray
            else
                [||]
