...
///
/// hover に出すのはこの行。表に書き写すと Core が動いたとき表だけが古びるので、
/// 腕の並びから組む —— `option` が `?`、`list` が `*`、多腕 DU が選択、
/// field の並びがそのまま順序になる。
///
/// --- 突き合わせる相手
///
/// 手書きの写しが doc コメントに在る（`DTD.fs` と `BulletmlRead.fs`）。
/// **生成とは別に書かれたもの**なので、独立な 2 本 として突き合わせられる。
/// XML doc file（`FsBulletML2.Core.xml`）から読む —— ソースの場所を
/// 試験が知らずに済む。
///
/// 手書きの写しは repo に 4 組 在る（上の 2 つ に加えて `OtherParse.fs` /
/// `XmlParse.fs` の見出し）。後ろの 2 組 は `//` なので doc に載らない。
/// **ここで見るのは doc に載る 2 組 だけ。**
///
/// --- 空白を潰して比べる
///
/// 写しどうしが既に空白で食い違っている（Core 側は `(param* )`、
/// 試験側は `(param*)`）。**空白は写した人の癖**なので潰す。
/// 潰すのは連なりと、括弧の内側に寄った 1 つ だけ。
///
/// --- 食い違いは表に載せる。ただし「まだ食い違っていること」も見る
///
/// 除外の表は放っておくと古びる（直ったのに載ったまま）。だから
/// **表に載っている相手が、いま本当に食い違っているか**を別の点で見る。
/// 一致したら赤 —— そのとき消すのは実装ではなく表。
///
/// --- 0 件 を緑にしない
///
/// 生成が空 / 手書きが空 / 一致が 0 は、突き合わせが「両方 空で緑」に
/// なる形。それぞれ別の点に置く。
///
/// --- 較正（当てた変異と、赤くなった点）
///
///   生成から ? を落とす                 両方に在る行は一致する
///   食い違いの表から vanish を外す       両方に在る行は一致する
///   vanish の差を消す（表は残したまま）  表に載っているものは本当に食い違っている
///   写しの片方 だけを書き換える          写しどうしが食い違っていない
///   拡張の表から name を外す             同じ相手を並べている
///
/// 最後に doc file を隠して回すと 4 点 赤。**「両方に在る行は一致する」は
/// そのとき緑のまま**（比べる相手が 0 件 なので）。0 件 を別の点に置いた
/// のはこの形のため。
[<TestFixture>]
type DtdLines() =
  ...
  static let generated =
    [ for e in Vocabulary.elements do
        yield e.Dtd
        for a in e.Attrs do yield a.Dtd ]
    |> List.map norm
    |> List.distinct
  ...
  static let knownDifferent =
    [ "action",
      "F# の DU にはヘッジが無い。手書きは (fire | fireRef) / (action | actionRef) と括るが、Action の腕は平らに並んでいる"
      "vanish",
      "手書きは (#PCDATA)。読む側が中身を許すのを写したもので、腕は持ち物を持たない"
      "bulletml/@type",
      "手書きは \"none\"、実装が属性なしのとき返すのは vertical。RELAX 定義に既定値は無いので、どちらが原文かを辿れない" ]
    |> Map.ofList
  ...
  static let handByKey =
    lazy (hand.Value |> List.groupBy key |> List.map (fun (k, v) -> k, List.distinct v) |> dict)
  ...
  member _.``生成と手書きが、同じ相手を並べている``() =
    ...
    Set.difference (Set.difference g h) generatedOnly |> Set.toList |> should be Empty
  ...
  member _.``食い違いの表に載っているものは、いま本当に食い違っている``() =
    ...
    let stale =
      [ for KeyValue(k, why) in knownDifferent do
          ...
          | (true, h), (true, g) when g = List.head h ->
            yield sprintf "%s: 一致した。表から外すこと（載せていた理由: %s）" k why
          | (false, _), _ ->
            yield sprintf "%s: 手書きに無い。表から外すこと" k
          | _, (false, _) ->
