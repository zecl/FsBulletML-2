...
type Transcode() =
    ...
    static let kinds =
        [ SourceKind.Xml; SourceKind.Sxml; SourceKind.Fsb; SourceKind.FSharpDsl ]
            ...
            [
                for kind in kinds do
                    for info in catalog do
                        ...
                        | Result.Error why ->
                            yield sprintf "%s / %s: %s" kind.Id (defaultArg info.Bulletml.Name "?") why
                        | Result.Ok _ -> ()
            ]
                ...
                for kind in kinds do
                    for info in catalog do
                        ...
                        | Result.Ok once ->
                            ...
                            | Result.Ok _ ->
                                yield sprintf "%s / %s: 2 回 目 で動いた" kind.Id (defaultArg info.Bulletml.Name "?")
                            | Result.Error why ->
                                yield sprintf "%s / %s: %s" kind.Id (defaultArg info.Bulletml.Name "?") why
                        | Result.Error _ -> ()
            ]
    ...
    member _.``fsb は式の空白を落とす``() =
        ...
        match
            SourceWriter.fsb.Write(
                Bulletml.readXmlString "<bulletml><action label=\"top\"><wait>90 * 2</wait></action></bulletml>"
            )
        with
    ...
    member _.``既存の sxml コーパスは そのまま読める``() =
        ...
        let readable =
            files
            |> Array.filter (fun f -> (SourceReader.sxml.Apply ignore (File.ReadAllText f)).IsNone)
    ...
    member _.``CE で description は 書けないと言う``() =
        ...
        let src =
            "<bulletml description=\"あ\"><action label=\"top\"><wait>1</wait></action></bulletml>"
