...
type PublicSurface() =
    ...
    let isEnumerable (t: Type) =
        let rec walk (t: Type) =
            if t = null then
                false
            elif t.IsByRef || t.IsArray || t.IsPointer then
                walk (t.GetElementType())
            elif
                t.IsGenericType
                && (t.GetGenericTypeDefinition() = typedefof<IEnumerable<_>>
                    || t.GetGenericTypeDefinition() = typedefof<IEnumerator<_>>)
            then
                true
            elif t.IsGenericType then
                t.GetGenericArguments() |> Array.exists walk
            else
                false
    ...
    let offenders (t: Type) =
        ...
        [
            for m in t.GetMethods(flags) do
                if isEnumerable m.ReturnType then
                    yield sprintf "%s.%s -> %s" t.Name m.Name m.ReturnType.Name

                for p in m.GetParameters() do
                    if isEnumerable p.ParameterType then
                        yield sprintf "%s.%s(%s)" t.Name m.Name p.Name
            for p in t.GetProperties(flags) do
                if isEnumerable p.PropertyType then
                    yield sprintf "%s.%s" t.Name p.Name
            for c in t.GetConstructors(flags) do
                for p in c.GetParameters() do
                    if isEnumerable p.ParameterType then
                        yield sprintf "%s..ctor(%s)" t.Name p.Name
        ]
