...
/// 乱数は同じ列を 2 本 用意する。1 本を分け合うと二重に消費する。
[<TestFixture>]
type DriverEquiv() =
    ...
    let directEnv
        (rand: unit -> float32)
        (target: float32 -> float32 -> ValueOption<struct (float32 * float32)>)
        (flip: bool)
        (spawnAtOrigin: bool)
        (x: float32)
        (y: float32)
...

        {
            ...
            Aim =
                {
                    ToPlayer = atan2f x y playerX playerY flip
                    ToEnemy = toEnemy
                }
            Spawn =
                {
                    ToPlayer = atan2f sx sy playerX playerY flip
    ...
    let runThroughFront
        ...
        all.Add
            {
                Run = Runner.newRoot BulletType.Enemy script
                X = 0.0f
                Y = 0.0f
                Alive = true
                Id = 0
        ...
        for i in 0 .. frames - 1 do
            ...
            for j in 0 .. liveCount - 1 do
                ...
                if b.Alive then
                    ...
                    sb.AppendLine(
                        sprintf
                            "  b%d x=%.6f y=%.6f d=%.6f s=%.6f%s"
                            nb.Id
                            nb.X
                            nb.Y
                            f.Run.Motion.Dir
                            f.Run.Motion.Speed
                            (if f.Vanished then " vanish" else "")
                    )
                    |> ignore
    ...
    let runDirect (flip: bool) (spawnAtOrigin: bool) (rand: unit -> float32) (xml: string) (frames: int) : string =
        let script = Runner.load rand 0.5f (Bulletml.readXmlString xml)
        let all = List<Live>()
        ...
        for i in 0 .. frames - 1 do
            ...
            for j in 0 .. liveCount - 1 do
                ...
                if b.Alive then
                    ...
                    let env =
                        if b.Run.HasNoScript then
                            noAim
                        else
                            directEnv rand t flip spawnAtOrigin b.X b.Y
                    ...
                    nb.Run <-
                        if f.Finished then
                            let renv =
                                if f.Run.HasNoScript then
                                    noAim
                                else
                                    directEnv rand t flip spawnAtOrigin nb.X nb.Y
    ...
    let compareAll (space: Space) (origin: SpawnOrigin) (flip: bool) (spawnAtOrigin: bool) =
        ...
        if not (List.isEmpty diverged) then
            Assert.Fail(sprintf "%d 本 で軌跡が割れた。最初: %s" (List.length diverged) (List.head (List.rev diverged)))
