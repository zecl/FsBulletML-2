...
type BudgetTests() =
    ...
    member _.``既定 は いま までと 同じ``() =
        let xml (b: BulletmlInfo) =
            BulletmlWriter.toIndentedXml 2 b.Bulletml

        xml (Generate.generate heavy)
        |> should equal (xml (Generate.generateTo (float Consts.MAX_ALIVE) heavy))

        xml (Harmonic.generate flower)
        |> should equal (xml (Harmonic.generateTo (float Consts.MAX_ALIVE) flower))
    ...
    member _.``取り分 を半分 にすると 同時数 が減る``() =
        ...
        let half =
            aliveOf 600 (Generate.generateTo (float Consts.MAX_ALIVE / 2.0) heavy).Bulletml
    ...
    member _.``花 も 取り分 で減る``() =
        ...
        let half =
            aliveOf 600 (Harmonic.generateTo (float Consts.MAX_ALIVE / 2.0) flower).Bulletml
