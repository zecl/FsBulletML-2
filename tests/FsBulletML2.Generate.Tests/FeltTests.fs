...
type FeltTests() =
    ...
    static let cohort (frame: int) (born: int) (pts: (float * float) list) : Felt.Snapshot =
        {
            Felt.Frame = frame
            Felt.Positions = pts
            Felt.Born = List.replicate pts.Length born
            Felt.Headings = []
            Felt.Speeds = []
        }
    ...
    static let runOf frames kind facing =
        Felt.run frames (Generate.generate (named kind facing)).Bulletml
    ...
    static let near (target: float) (hs: float list) =
        hs
        |> List.filter (fun h -> abs (System.Math.IEEERemainder(h - target, twoPi)) <= deg 0.5)
        |> List.length
    ...
    static let fullest (snaps: Felt.Snapshot list) =
        snaps |> List.maxBy (fun s -> s.Headings.Length)

    /// 帯 は 群 が 8 コマ 飛んだ あと にしか 測れない ので、全 コマ の最大 を採る
    static let bestBand (snaps: Felt.Snapshot list) =
        snaps |> List.map Felt.bandScore |> List.max
    ...
    static let distinct (hs: float list) =
        ...
        if a.Length = 0 then
            ...
            let inner =
                a |> Array.pairwise |> Array.filter (fun (p, q) -> q - p > tol) |> Array.length
    ...
    static let arcDeg (hs: float list) =
        ...
        let gaps =
            Array.append [| a.[0] + twoPi - a.[a.Length - 1] |] (a |> Array.pairwise |> Array.map (fun (p, q) -> q - p))
    ...
    member _.``発射角 は 0..2π に畳む``() =
        let heads =
            Felt.run 90 (Generate.generate (spiral ())).Bulletml
            |> List.collect (fun s -> s.Headings)
    ...
    member _.``完全 な 3・4・8 方向 は ring が高い``() =
        for n in [ 3; 4; 8 ] do
            Felt.ringScore (shot [ for i in 0 .. n - 1 -> float i * twoPi / float n ] [])
            |> should be (greaterThan 0.9)

    [<Test>]
    member _.``90 度 の扇 は ring が低い``() =
        Felt.ringScore (shot [ for i in 0..8 -> float i * System.Math.PI / 16.0 ] [])
        |> should be (lessThan 0.3)
    ...
    member _.``band は 生まれて 8 コマ 未満 の群 と 8 発 未満 の群 を見ない``() =
        let line n =
            [ for i in 0 .. n - 1 -> 100.0 + float i * 30.0, 300.0 ]
    ...
    member _.``band は 群 を混ぜない``() =
        ...
        let s: Felt.Snapshot =
            {
                Felt.Frame = 20
                Felt.Positions = List.map fst rows
                Felt.Born = List.map snd rows
                Felt.Headings = []
                Felt.Speeds = []
            }
    ...
    member _.``狙い の方位 に揃う と aim が高く、真裏 は 0``() =
        Felt.aimScore 240.0 600.0 (shot (List.replicate 5 System.Math.PI) [])
    ...
    member _.``recip は ほぼ 0 の弾 を 数 から 落とす``() =
        ...
        let speeds =
            heads
            |> List.mapi (fun i t -> if i < 3 then 1e-9 else 1.0 / (1.0 + 0.85 * cos (5.0 * t)))
    ...
    member _.``扇 は 名指し が無ければ 下 へ開く``() =
        ...
        abs (System.Math.IEEERemainder(c - System.Math.PI, twoPi))
        |> should be (lessThanOrEqualTo (deg 0.5))
    ...
    member _.``狙い は自機 方位 に寄る``() =
        ...
        let three =
            fullest (Felt.run 90 (Generate.generate (namedWays 3 Aimed Around)).Bulletml)
    ...
    member _.``狙い は自機 を追い、幕 は追わない``() =
        let centerAt px kind =
            centerOf (fullest (Felt.runWith px 600.0 90 (Generate.generate (named kind Around)).Bulletml))

        let shift kind =
            abs (System.Math.IEEERemainder(centerAt 60.0 kind - centerAt 240.0 kind, twoPi))

        abs (shift Aimed - atan (180.0 / 520.0))
        |> should be (lessThanOrEqualTo (deg 1.0))
    ...
    member _.``runAt は 難度 を 渡す``() =
        ...
        let shots rank =
            Felt.runAt rank 3 b |> List.sumBy (fun s -> s.Headings.Length)
