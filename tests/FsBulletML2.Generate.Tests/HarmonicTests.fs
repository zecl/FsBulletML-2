...
type HarmonicTests() =
  ...
  // --- 較正（当てた変異 と、赤 になった 点）
  //
  //   create の layers の max 1 を外す        指紋 ほか 35 本（Layers 0 で 割り算 が落ちる）
  //   layerShift を 常 に 0                   層 は 同じ 角 に… / ハート の 層 は 半周 ずれる
  //   Arms を 層 で割らない                   層 は 1 波 の 弾数 を… / 層 は 字 の値段 を…
  //   symmetryOf の gcd を Folds に           互いに素 の 第 2 の波 は 畳めない
  ...
  member _.``種 は 18 通り を 巡る``() =
    let of_ seed =
      ...
      h.Folds2, h.Amplitude2, h.Layers, h.LayerPhase
  ...
  [<Test>]
  member _.``層 は 1 波 の 弾数 を 増やさない``() =
    let mk layers = petal5 (fun a -> { a with Layers = layers })
    // 畳んだ 波 は 頭 が 1 発 重なる。層 を足す と その 1 発 も 層 の数 だけ 増える
    Harmonic.shotsPerWave (mk 2) |> should be (lessThanOrEqualTo (Harmonic.shotsPerWave (mk 1) + 1))
    Harmonic.shotsPerWave (mk 2) |> should be (greaterThan (Harmonic.shotsPerWave (mk 1) - 10))
  ...
  [<Test>]
  member _.``層 は 字 の値段 を上げない``() =
    let sizeOf layers =
      let h = petal5 (fun a -> { a with Folds2 = 8; Amplitude2 = 0.4; Layers = layers })
      (BulletmlWriter.toIndentedXml 2 (Harmonic.generate h).Bulletml).Length
    for layers in [ 2; 3 ] do
      float (sizeOf layers) / float (sizeOf 1) |> should be (lessThan 1.2)
