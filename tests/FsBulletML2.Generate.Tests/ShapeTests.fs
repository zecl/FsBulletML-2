...
type ShapeTests() =
    ...
    member _.``段 が出る``() =
        for n in 0..3 do
            countOf "<bullet label=" (toXml (Generate.generate (thin (float n))))
            |> should equal (n + 1)
    ...
    member _.``層 が出る``() =
        for n in 0..2 do
            let x = toXml (Generate.generate (spec (fun a -> { a with Layers = float n })))
    ...
    member _.``段 が入れ子``() =
        for n in 0..2 do
            let x = toXml (Generate.generate (spec (fun a -> { a with Depth = float n })))
            countOf "<repeat>" x |> should equal (n + 2)

    [<Test>]
    member _.``間 が出る``() =
        let withPause = toXml (Generate.generate (spec (fun a -> { a with Pause = true })))
    ...
    [<Test>]
    member _.``引数違い で呼ぶ``() =
        let x = toXml (Generate.generate (spec (fun a -> { a with Parametrized = true })))
    ...
    member _.``repeat の回数 が上限 以下``() =
        ...
        for m in Regex.Matches(x, @"<times>([^<]+)</times>") do
            evalAt 1.0 m.Groups.[1].Value
            |> should be (lessThanOrEqualTo (float Consts.MAX_REPEAT))
    ...
    member _.``幕 は帯 を等間隔 に掃く``() =
        ...
        x
        |> should haveSubstring (sprintf "<direction type=\"absolute\">180 - %d</direction>" (curtainSpan d))
        // 刻み は 帯 の幅 を腕 で割った もの。`360 /` が出たら 全周 を掃いて いる
        x
        |> should haveSubstring (sprintf "<direction type=\"sequence\">%d / (" (curtainSpan d * 2))
    ...
    member _.``届かない 本数 は 名指し しない``() =
        ...
        (named Radial PatternSpec.MAX_WAYS Around Plain).Ways
        |> should equal PatternSpec.MAX_WAYS
