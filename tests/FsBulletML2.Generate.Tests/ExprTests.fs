...
type ExprTests() =
    ...
    member _.``扇 が 1 本 なら 頭 は中心``() =
        ...
        fanHead "0" 30 (spec (fun a -> { a with Kind = Aimed; Ways = 1 }))
        |> should equal "0"
    ...
    member _.``段 が深い ほど 1 段 の予算 が小さい``() =
        ...
        evalAt 1.0 (scatterExpr three 0)
        |> should be (lessThan (evalAt 1.0 (scatterExpr one 0)))
    ...
    member _.``WaitScale が wait を伸ばす``() =
        ...
        evalAt 0.0 (waitExpr scaled 0)
        |> should be (greaterThan (evalAt 0.0 (waitExpr s 0)))
