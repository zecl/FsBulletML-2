...
/// という形で、**Single.Parse にカルチャを渡していなかった**。
/// XPath が返すのは "." 区切りなので、"," が小数点のカルチャだと
/// 解釈が変わるはず、という読み（未測定の読みとして立てたもの）。
///
/// **その評価器は Core に無い**（v5.5 で `XPathOracle.fs` へ移した）。
/// が、**当てる先は消えていない** —— いまの道にも字と数の行き来が 2 か所 在る。
///
///     読む   `Expr.fs` が数の字を読む（Double.TryParse）
///     畳む   `BulletmlRead.foldConstants` が数を字へ書き戻す（ToString）
///
/// どちらも不変カルチャを明示で渡してある。
/// **明示が効いていることを、ここで実際に走らせて確かめる。**
[<TestFixture>]
type Culture() =
