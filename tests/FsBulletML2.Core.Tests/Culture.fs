...
<action label="top">
  ...
  <wait>10</wait>
    ...
    member _.``小数の出る式を、小数点がカンマのカルチャで評価する``() =
        ...
        let row name =
            under name (fun () -> exprs |> List.map (fun e -> sprintf "%s=%s" e (speedOf e)) |> String.concat "  ")
            |> sprintf "%-8s %s" name
  ...
  <fire>
    ...
    member _.``カルチャごとの数の書き方``() =
        ...
        let row (name: string) (f: NumberFormatInfo) =
            sprintf
                "%-10s 小数点 %-8s 桁区切り %-8s 負号 %s"
                name
                (esc f.NumberDecimalSeparator)
                (esc f.NumberGroupSeparator)
                (esc f.NegativeSign)
