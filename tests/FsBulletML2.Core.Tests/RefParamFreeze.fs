...
<action label="shoot">
  ...
  <wait>1</wait>
    ...
    /// フレーム 4 で値を切り替える。
    let runSwitching (xml: string) (rand: unit -> float32) (rank: unit -> float32) (switch: unit -> unit) =
...
<action label="fan">
  ...
  <fire><direction type="absolute">$1</direction><speed>3</speed><bullet/></fire>
        ...
        TraceApi.runWithParams hook (fun () -> rand) (fun () -> 0.5f) (fun () -> 30.0f) (fun () -> 100.0f) xml 4
        |> fun t ->
            t.Split('\n')
            |> Array.filter (fun l -> l.Contains "  +b")
            |> Array.map (fun l -> l.Trim())
