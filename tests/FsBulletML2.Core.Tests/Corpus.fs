...
module internal CorpusData =
    ...
    let progressPath = Path.Combine(Path.GetTempPath(), "fsb-corpus-progress.txt")
...
type Corpus() =
        ...
        let lines =
            [
                ...
                if List.isEmpty skipped then
                    ...
                    for r in skipped do
                        yield sprintf "  %s" r.Name
                yield ""
                yield "落ちたもの:"
                if List.isEmpty broke then
                    yield "  なし"
                else
                    for r in broke do
    ...
    member _.``samples の弾幕の軌跡を 1 本ずつ控えにする``() =
        ...
        [
            ...
            yield sprintf "軌跡の指紋は %d 種類（%d 本は、中身が違うのに軌跡が同じ）" kinds (List.length ran - kinds)
            yield
                sprintf
                    "撃った弾の合計 %d 発 ／ 最終フレームに残っていた合計 %d 本"
                    (ran |> List.sumBy (fun r -> r.Fired))
                    (ran |> List.sumBy (fun r -> r.Alive))
            yield ""
            yield! body
            yield ""
    ...
    member _.``227 本の走行が桁で遅くなっていない``() =
        ...
        Assert.That(ms, Is.LessThan 120000.0, sprintf "227 本の走行に %.0f ms 掛かっています。桁で遅くなっていないか見てください" ms)
