...
type TuneThin() =
    ...
    member _.``寿命 が付く のは 終点 の弾 だけ``() =
        ...
        got.IndexOf "tuned-thin"
        |> should be (greaterThan (got.IndexOf "<wait>20</wait>"))
    ...
    member _.``もう 1 段 薄く する と 寿命 が短く なる``() =
        ...
        System.Text.RegularExpressions.Regex.Matches(got, "tuned-thin").Count
        |> should equal 1
    ...
    member _.``押し戻す と 元 の字 に戻る``() =
        ...
        xml (Tune.apply Tune.Thicker (Tune.apply Tune.Thinner b))
        |> should equal (xml b)
    ...
    member _.``段 を足して から 間引く``() =
        ...
        both
        |> should equal (xml (Tune.apply Tune.Thinner (Tune.apply Tune.AddSplit b)))

        both
        |> should not' (equal (xml (Tune.apply Tune.AddSplit (Tune.apply Tune.Thinner b))))
    ...
    member _.``数 を振って から 間引く``() =
        ...
        both
        |> should not' (equal (xml (Tune.apply Tune.Denser (Tune.apply Tune.Thinner b))))
    ...
    member _.``薄く する と 同時 に居る 弾 が減る``() =
        ...
        if before = 0 then
            Assert.Fail "素 の弾幕 が 1 発 も 残って いません。この点 は 何 も 測って いません"
