...
/// 控えが無いときは書いて落とす。黙って作って緑にすると、何も担保していない。
module Golden =
    ...
    let private read (name: string) =
        ...
        if File.Exists fromSource then
            Some(File.ReadAllText fromSource |> normalize)
        elif File.Exists fromOutput then
            Some(File.ReadAllText fromOutput |> normalize)
        else
            None
    ...
    let check (name: string) (actual: string) =
        ...
        | None ->
            ...
            Assert.Fail(
                sprintf
                    "控えがありませんでした。いまの実装の出力を %s に書きました。\n\
        ...
        | Some expected ->
            if expected <> actual then
                ...
                Assert.Fail(
                    sprintf "軌跡が控えと違います（%s）。\n%s\n\n--- 控え ---\n%s\n\n--- いま ---\n%s" name where expected actual
                )
