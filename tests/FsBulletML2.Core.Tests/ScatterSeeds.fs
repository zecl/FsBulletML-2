...
type ScatterSeeds() =
    ...
    member _.``茎 が n 角形 の中心 を 下 へ運ぶ``() =
        ...
        topPart s
        |> should haveSubstring "<direction type=\"absolute\">180.00</direction>"
        ...
        for n in [ 2; 3; 5; 8 ] do
            let up =
                [ 0 .. n - 1 ]
                |> List.map (fun j -> Scatter.REACH * cos (Scatter.vertexDeg n j * System.Math.PI / 180.0))
    ...
    member _.``名前 がぶつかったら 番号 を足す``() =
        let taken = ring + """<bullet label="scattered-seed"><speed>1</speed></bullet>"""
