...
module internal ExprCorpus =
    ...
    let private loadDoc (path: string) =
        let settings =
            XmlReaderSettings(DtdProcessing = DtdProcessing.Ignore, XmlResolver = null)
    ...
    let paramUsers: Lazy<string list> =
        lazy
            (all.Value
             |> List.filter (fun s -> Text.RegularExpressions.Regex.IsMatch(s, @"\$\d")))
...
type ExprTests() =
    ...
    member _.``samples から式が集まっている``() =
        ...
        Assert.That(
            List.length exprs,
            Is.GreaterThan 500,
            sprintf "式が %d 種 しか集まっていません。走査が壊れています（%s）" (List.length exprs) CorpusData.samplesDir
        )

        TestContext.WriteLine(
            sprintf
                "式 %d 種 / うち $N を使うもの %d 種 / 実引数 %d 種"
                (List.length exprs)
    ...
    member _.``実物の式ぜんぶで getValueByXPath と同じ値になる``() =
        ...
        for s in exprs do
            ...
            for randValue, rank in ExprCorpus.cases do
                ...
                | ExprCorpus.Threw name -> threw.Add(sprintf "  [%s]  rand=%g rank=%g  旧が %s" s randValue rank name)
                | ExprCorpus.Value expected ->
                    if not (ExprCorpus.same expected actual) then
                        diffs.Add(sprintf "  [%s]  rand=%g rank=%g  旧 %.9g / 木 %.9g" s randValue rank expected actual)

        TestContext.WriteLine(
            sprintf
                "突き合わせ %d 通り（式 %d 種 x 振り %d 通り）／旧が落ちたのは %d 通り"
                checkedCount
                (List.length exprs)
                (List.length ExprCorpus.cases)
                threw.Count
        )
    ...
    member _.``実引数を差し込んだ形でも getValueByXPath と同じ値になる``() =
        ...
        let hasTopLevelAddSub (s: string) =
            ...
            for c in s do
                if c = '(' then
                    ...
                    if Char.IsDigit prev || prev = ')' || Char.IsLetter prev then
                        hit <- true
            ...
            for arg in chosenArgs do
                ...
                for randValue, rank in [ 0.5f, 0.5f; 0.123456f, 0.987654f ] do
                    ...
                    if not (ExprCorpus.same expected actual) then
                        diffs.Add(
                            sprintf
                                "  仮[%s] 実[%s] -> [%s]  rand=%g rank=%g  旧 %.9g / 木 %.9g"
                                user
                                arg
                                substituted
                                randValue
        ...
        TestContext.WriteLine(
            sprintf
                "差し込んで突き合わせ %d 通り（仮 %d x 実 %d x 振り 2）"
                checkedCount
                (List.length chosenUsers)
                (List.length chosenArgs)
        )
    ...
    member _.``突き合わせは優先順位の間違いを拾える``() =
        ...
        for s in exprs do
            ...
            if not (ExprCorpus.same expected broken) then
                caught <- caught + 1
        ...
        Assert.That(caught, Is.GreaterThan 300, sprintf "壊した評価器が %d 種 でしか赤くなりません。突き合わせが違いを拾えていない疑いがあります" caught)
    ...
    member _.``小さい rand rank で getValueByXPath は落ちる``() =
        ...
        TestContext.WriteLine(
            sprintf
                "旧が落ちた値: %s"
                (crashed
                 |> List.map (fun v -> v.ToString(Globalization.CultureInfo.InvariantCulture))
                 |> String.concat ", ")
        )

        Assert.That(List.length crashed, Is.GreaterThan 0, "旧が 1 つも落ちません。この穴が塞がったなら、この試験を消してよい")
    ...
    member _.``読めない式は 旧が例外 木は NaN``() =
        ...
        Assert.That(currentThrew, Is.GreaterThan 0, "旧が 1 つも落ちません。この試験が前提にしている差が無くなっています")

    /// 上の差 2 が同梱の台本で踏めないことを、数で押さえる。
    /// 読めない式が 1 つでもあれば、そこで旧と木の振る舞いが分かれる
    [<Test>]
    member _.``同梱の台本に読めない式は無い``() =
        let bad = ExprCorpus.all.Value |> List.filter (fun s -> Expr.parse s = Expr.Invalid)
    ...
    member _.``NumExpr はもとの文字列を保つ``() =
        ...
        let replaced =
            e |> Expr.NumExpr.mapSource (fun s -> Param.replace s (Param.ofList [ "1+2" ]))
        ...
        Assert.That(float v, Is.EqualTo(1.0 + 2.0 * 0.75).Within(1e-6), "文字の置き換えとして読めていません（木の節として差し込むと 2.25 になります）")
    ...
    member _.``木の評価は getValueByXPath より桁で速い``() =
        ...
        for _ in 1..n do
            acc <- acc + Expr.NumExpr.evalWithValues 0.5f 0.5f e
