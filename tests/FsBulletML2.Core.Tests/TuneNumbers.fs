...
type TuneNumbers() =
  ...
  static let knobs =
    [ Tune.Faster; Tune.Slower; Tune.Denser; Tune.Sparser
      Tune.Spinnier; Tune.Steadier; Tune.MoreArms; Tune.FewerArms
      Tune.MoreAccel; Tune.LessAccel; Tune.Snappier; Tune.Lazier ]
  ...
  member _.``軸 の名 が 10 通り 別 に読める``() =
    let names =
      [ "speed"; "density"; "spin"; "arms"; "accel"; "term"; "split"; "layer"; "thin"; "hush" ]
    ...
    let structural =
      [ Tune.AddSplit; Tune.DropSplit; Tune.AddLayer; Tune.DropLayer
        Tune.Thinner; Tune.Thicker; Tune.Hush; Tune.Unhush ]
  ...
  member _.``段数 を まとめて 当てる``() =
    ...
    xml (Tune.applySteps [ "speed", 1 ] b) |> should equal (xml (Tune.apply Tune.Faster b))
    xml (Tune.applySteps [ "speed", -1 ] b) |> should equal (xml (Tune.apply Tune.Slower b))
    ...
    both |> should not' (equal (xml (Tune.apply Tune.AddSplit (Tune.apply Tune.Faster b))))

  /// 頼んだ 相手 が どんな 数 を返して も、当てる 数 は こちら が決める
  [<Test>]
  member _.``段数 は 上限 で止まる``() =
    let b = Bulletml.ReadXmlString(wrap plain)
    let rec press i (x: Bulletml) = if i = 0 then x else press (i - 1) (Tune.apply Tune.Faster x)
    xml (Tune.applySteps [ "speed", 99 ] b) |> should equal (xml (press Tune.MAX_STEPS b))
  ...
  member _.``名前 で引く 弾 にも 段 を足せる``() =
    ...
    xml (Tune.apply Tune.DropSplit (Tune.apply Tune.AddSplit b)) |> should equal (xml b)

  /// 段 は 掛け算 で増える。青天井 だと 3 段 目 で 面 が止まる
  [<Test>]
  member _.``段 と 層 は 上限 で止まる``() =
    let b = Bulletml.ReadXmlString(wrap plain)
    let rec press knob n (x: Bulletml) = if n = 0 then x else press knob (n - 1) (Tune.apply knob x)
