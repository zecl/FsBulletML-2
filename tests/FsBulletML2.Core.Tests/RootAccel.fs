...
type RootAccel() =
  ...
  let accelScript =
    RecBulletml.Accel (
      Some (Horizontal (Some { horizontalType = HorizontalType.Absolute }, "2")),
      Some (Vertical (Some { verticalType = VerticalType.Absolute }, "1")),
      Term "5")
  ...
  member _.``top 直下の accel は、3 コマ動いても軌跡を動かさない``() =
    let top = RecBulletml.Action ({ actionLabel = Some "top" }, [ accelScript; RecBulletml.Wait "20" ])
