...
type StepFire() =
    ...
    let env =
        {
            Rand = (fun () -> 0.5f)
            Rank = 0.5f
            Aim = { ToPlayer = 1.0f; ToEnemy = 2.0f }
            Spawn = { ToPlayer = 3.0f; ToEnemy = 4.0f }
        }
    ...
    member _.``fire は、撃たれた弾を 1 つ出す``() =
        ...
        let (r, _, _), _, w =
            Sim.run env state (stepFire noResolvers script (PFire false) FireContext.zero)
    ...
    member _.``bullet 側の aim は、産まれる弾の位置から見た向きで解決する``() =
        ...
        | [ Spawn b ] ->
            ...
            b.Dir
            |> should (equalWithin 0.0001) (Step.calcDir (env.Spawn.ToPlayer + float32 (System.Math.PI / 6.0)))
        | _ -> Assert.Fail(sprintf "Spawn 1 つのはずが %A" w)
    ...
    member _.``sequence は、直前の fire の値に積む``() =
        ...
        let (_, _, fc'), _, _ =
            Sim.run env state (stepFire noResolvers script (PFire false) fc)
        // 10 度 = π/18 を足す
        fc'.SrcDir
        |> should (equalWithin 0.0001) (1.0f + float32 (System.Math.PI / 18.0))

    [<Test>]
    member _.``bulletRef は 1 段だけ解く``() =
        let target =
            bullet None (Some(Speed(Some { speedType = SpeedType.Absolute }, numExpr "7")))
        ...
        let _, _, w =
            Sim.run env state (stepFire resolvers script (PFire false) FireContext.zero)
    ...
    member _.``bulletRef を解いた瞬間にも、bullet 本体の中の wait を先に引く（resetChild と合わせて 2 回）``() =
        ...
        let _, _, w =
            Sim.run counting state (stepFire resolvers script (PFire false) FireContext.zero)
    ...
    member _.``撃つ側が Player なら、direction を省くと Aim.ToEnemy になる``() =
        ...
        let _, _, w =
            Sim.run env playerState (stepFire noResolvers script (PFire false) FireContext.zero)
    ...
    member _.``fire は、撃たれた弾の action の中身の getValue も先に引く``() =
                ...
                [
                    ActionElm.Action(
                        ...
                        [
                            ...
                            Action.ChangeSpeed(
                                Speed(Some { speedType = SpeedType.Absolute }, numExpr "5"),
                                Term(numExpr "4")
                            )
                        ]
                    )
        ...
        Sim.run counting state (stepFire noResolvers script (PFire false) FireContext.zero)
        |> ignore
