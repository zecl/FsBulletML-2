...
type SimTests() =
    ...
    member _.``モナド則: 結合則 —— 繋ぐ順を変えても同じ``() =
        let f (x: float32) =
            simForTests {
                do! Sim.emit (Spawn { st0 with Speed = x })
                return x + 1.0f
            }

        let g (x: float32) =
            simForTests {
                do! Sim.put { st0 with Speed = x }
                return x * 3.0f
            }
    ...
    member _.``BindReturn は Bind + Return と同じ答えを返す``() =
        same (simForTests.BindReturn(moving, fun x -> x * 2.0f)) (simForTests.Bind(moving, fun x -> Sim.ret (x * 2.0f)))
