...
<fire label="f">
  ...
  <bullet/>
    ...
    let firstBullet (xml: string) =
        ...
        if m.Success then
            sprintf "d=%s s=%s" m.Groups.[1].Value m.Groups.[2].Value
        else
            "撃っていない"
...
<action label="top">
  ...
  <wait>10</wait>
    ...
    member _.``rand と rank が式に混ざる``() =
        ...
        [ "$rand"; "$rank"; "$rand+$rank"; "1+$rand*2"; "$rand*$rank*8" ]
    ...
    member _.``実物 2 本の 未解決の パラメータ``() =
        let names =
            [ "[Bulletsmorph]_kunekune_plus_homing.xml"; "[Bulletsmorph]_satoru4.xml" ]

        let dir =
            System.IO.Path.GetFullPath(System.IO.Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "samples"))
        ...
        let rows =
            ...
            |> List.map (fun n ->
                ...
                | Some p ->
                    let trace =
                        try
                            TraceRun.withRandRank 0.5f 0.25f (System.IO.File.ReadAllText p) 200
                        with e ->
                            let rec inner (x: exn) =
