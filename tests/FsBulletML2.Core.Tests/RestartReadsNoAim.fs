...
type RestartReadsNoAim() =
    ...
    member _.``227 本 とも、aim を変えても走らせ直しの結果は同じ``() =
        ...
        if not (List.isEmpty diverged) then
            Assert.Fail(sprintf "%d 本 で割れた。restart は aim を読んでいる。最初: %s" (List.length diverged) (List.head diverged))
    ...
    member _.``較正: 乱数を変えると走らせ直しの結果は動く``() =
        ...
        TestContext.WriteLine(sprintf "%d 本 中 %d 本 が乱数で動いた" compared (List.length diverged))
