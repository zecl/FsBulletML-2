    ...
    let run (rand: unit -> float32) (rank: float32) (px: float32) (py: float32) (xml: string) (frames: int) : string =
        ...
        for i in 0 .. frames - 1 do
                    ...
                    let env =
                        {
                            ...
                            Aim =
                                {
                                    ToPlayer = aimDir px py b.St.Pos
                                    ToEnemy = enemyAimDir b.St.Pos
                                }
                            Spawn =
                                {
                                    ToPlayer = spawnAim
                            ...
                            let reinitEnv =
                                {
                                    ...
                                    Aim =
                                        {
                                            ToPlayer = aimDir px py st.Pos
                                            ToEnemy = enemyAimDir st.Pos
                                        }
                                    Spawn =
                                        {
                                            ToPlayer = spawnAim
            ...
            while seen < all.Count do
                ...
                sb.AppendLine(sprintf "  +b%d d=%s s=%s" b.Id (fmt b.St.Dir) (fmt b.St.Speed))
                |> ignore
