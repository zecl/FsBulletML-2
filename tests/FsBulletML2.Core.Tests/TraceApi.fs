...
module TraceApi =
    ...
    let runDetailed
        ...
        let envAt (x: float32) (y: float32) : Env =
            {
                ...
                Aim =
                    {
                        ToPlayer = aimDir (px ()) (py ()) x y
                        ToEnemy = enemyAimDir x y
                    }
                Spawn =
                    {
                        ToPlayer = spawnAim ()
                        ToEnemy = spawnEnemyAim ()
                    }
            }
        ...
        all.Add
            {
                Run = Runner.newRoot rootKind script
                X = 0.0f
                Y = 0.0f
                Alive = true
                Id = 0
        ...
        for i in 0 .. frames - 1 do
            ...
            for j in 0 .. liveCount - 1 do
                ...
                if b.Alive then
                    ...
                    sb.Append(
                        sprintf
                            "  b%d %s x=%s y=%s d=%s s=%s"
                            b.Id
                            mark
                            (fmt b.X)
                            (fmt b.Y)
                            (fmt f.Run.Motion.Dir)
                            (fmt f.Run.Motion.Speed)
                    )
                    |> ignore

                    if f.Vanished then
                        sb.Append(" vanish") |> ignore
            ...
            while seen < all.Count do
                ...
                sb.AppendLine(sprintf "  +b%d d=%s s=%s" b.Id (fmt b.Run.Motion.Dir) (fmt b.Run.Motion.Speed))
                |> ignore
    ...
    /// 値を動かさない走行。
    let run (rand: unit -> float32) (rank: float32) (px: float32) (py: float32) (xml: string) (frames: int) : string =
