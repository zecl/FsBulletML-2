...
type OfficialDtd() =
    ...
    static let isDtdLine (s: string) =
        s.StartsWith "<!ELEMENT" || s.StartsWith "<!ATTLIST"

    static let officialLines =
        if not (File.Exists dtdPath) then
            [||]
        else
            File.ReadAllLines dtdPath |> Array.map norm |> Array.filter isDtdLine
    ...
    static let attlists =
        ...
        |> Array.map (fun l ->
            ...
            let values =
                ...
                if v.Success then
                    v.Groups.[1].Value.Split '|' |> Array.map (fun s -> s.Trim())
                else
                    [||]
        ...
        | Some rec_ ->
            ...
            | Some p ->
                ...
                if FSharpType.IsUnion t then
                    ...
                    Error(sprintf "%s.%s は DU ではない" attrsName fieldName)
    ...
    [<Test>]
    member _.``公式 DTD が repo に在る``() = File.Exists dtdPath |> should be True
    ...
    member _.``取れる値が公式と一致する``() =
        let bad =
            ...
            |> Array.choose (fun (elem, attr, values, _) ->
                ...
                | Ok(caseNames, _) ->
                    let missing =
                        values |> Array.filter (fun v -> not (caseNames |> Array.exists (matches v)))

                    let extra =
                        caseNames
                        |> Array.filter (fun c -> not (values |> Array.exists (fun v -> matches v c)))

                    if missing.Length = 0 && extra.Length = 0 then
                        None
                    else
                        Some(elem, attr, sprintf "公式だけ %A / こちらだけ %A" missing extra))
    ...
    member _.``既定値が公式と一致する（承知の違いを除く）``() =
        let bad =
            ...
            |> Array.choose (fun (elem, attr, values, deflt) ->
                if known.ContainsKey((elem, attr)) then
                    None
                else
                    match duCasesOf elem attr with
                    | Error why -> Some(elem, attr, why)
                    | Ok(_, ourCase) ->
                        let ourValue =
                            ourCase |> Option.bind (fun c -> values |> Array.tryFind (fun v -> matches v c))

                        if deflt = ourValue then
                            None
                        else
                            Some(elem, attr, sprintf "公式 %A / こちら %A" deflt ourValue))
