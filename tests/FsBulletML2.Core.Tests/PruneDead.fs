...
///
/// 当てる 先 は 編集 の 途中 の木 —— 同梱 も 合成 の出力 も 完成品 なので、
/// あちら で 数えて も 0 件 しか 出ない（815 個 中 5 個 だった）。
/// `<actionRef>` を消した あと に 定義 が 浮く 形 を 手 で作って 当てる。
///
/// --- 較正（当てた変異 と、赤 になった 点数）
///
///   `hasEntry` を常 に true            6 点
///   `hasEntry` を常 に false          13 点（top が入口 は 全点 の前提）
///   不動点 を 1 周 で止める             2 点（軌跡 も 赤 ＝ 実害 が在る 穴 だった）
///   `labelsIn` が 入れ子 を見ない       1 点
///   `survivors` を 恒等 に             6 点
[<TestFixture>]
type PruneDead() =
  ...
  member _.``落とす 前後 で 軌跡 が 一致 する``() =
    ...
    if List.isEmpty victims then
      Assert.Fail(sprintf "samples %d 本 に 落ちる 枝 が 1 本 も ありません。この点 は 何 も 測って いません"
    ...
    for (name, src, b) in victims do
      ...
      let run (s: string) = try Some(TraceRun.std s 60) with _ -> None
    ...
    if checked' = 0 then
      Assert.Fail(sprintf "落ちる 枝 を持つ %d 本 が どれ も 走らず、軌跡 を 1 度 も 比べて いません"
                    (List.length victims))

    TestContext.Out.WriteLine(sprintf "samples %d 本 中 %d 本 に 落ちる 枝。%d 本 の軌跡 が 一致"
                                (List.length readable) (List.length victims) checked')
