    ...
    member _.``run の返り値の使われ方を固める``() =
        ...
        let lines =
            [
                ...
                yield
                    sprintf
                        "―― 本体 %d ファイル / %d 行（0 なら走査か網が壊れている）"
                        (List.length core)
                        (core |> List.sumBy (fun (_, _, h) -> List.length h))
            ]

        Assert.That(core |> List.sumBy (fun (_, _, h) -> List.length h), Is.GreaterThan 5, "当てる先が少なすぎます。網が壊れています")
