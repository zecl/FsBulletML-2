...
/// その形になっていないかを見ること。
///
/// **Playground の速い道は、この網に当たらない**（v4.9.1）。あちらは
/// 差分を**配列へ入れておいて、あとで足す**ので
///
///     vel.[i * 2] <- d.X            差分を覚える（`ConstantDelta` から）
///     p.[b] <- p.[b] + v.[a]        足す（`.X` / `.Y` も `Delta` も出ない）
///
/// 網には `.X` も `.Delta` も出てこない。**広げない** ——
/// 代わりに `Front.Tests/ConstStep.fs` が、**同梱を 2 通り の面で走らせて
/// 座標がビット一致すること**を見ている。あちらのほうが強い
/// （字の形ではなく答えを見るので）。
///
