...
type TuneHush() =
    ...
    static let flat = """<action label="top"><fire><speed>2</speed><bullet/></fire></action>"""
    ...
    member _.``黙らせる と いちばん 深い 段 が 包まれる``() =
        ...
        got.IndexOf "tuned-hush"
        |> should be (greaterThan (got.LastIndexOf "<wait>10</wait>"))
    ...
    member _.``数 を振って から 黙らせる``() =
        ...
        both
        |> should not' (equal (xml (Tune.apply Tune.MoreArms (Tune.apply Tune.Hush b))))
    ...
    member _.``黙らせる と 撃つ 弾 が減る``() =
        ...
        if before = 0 then
            Assert.Fail "素 の弾幕 が 1 発 も 撃って いません。この点 は 何 も 測って いません"
