...
type StepTop() =
  ...
  member _.``複数 top の効果は、top の並び順のまま出る``() =
    ...
    let fireTop = top [ RecBulletml.Fire ({ fireLabel = None }, None, None, bullet) ]
    let vanishTop = top [ RecBulletml.Vanish ]
  ...
  ///
  /// wait "1" は 1 コマ目で Stopped、2 コマ目で Ended になる（Step.wait 参照）。
  ...
  member _.``FireContext は次のコマへ持ち越される: 2 発めの sequence は 1 発めの速さに積む``() =
    let bullet d s = RecBulletml.Bullet ({ bulletLabel = None }, d, s, [])
    let fire1 =
  ...
  /// StepCommands.fs の門は Step.repeat を直接見ているが、ここは top* の
