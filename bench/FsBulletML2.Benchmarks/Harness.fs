...
module Harness =
    ...
    let private aimDirAt (x: float32) (y: float32) =
        float32 (
            System.Math.Atan2(float (BulletMLManager.GetPlayerPosX() - x), float -(BulletMLManager.GetPlayerPosY() - y))
        )
    ...
    let envCost (x: float32) (y: float32) : Domain.Env =
        {
            ...
            Aim =
                {
                    ToPlayer = aimDirAt x y
                    ToEnemy = enemyAimDirAt x y
                }
            Spawn =
                {
                    ToPlayer = aimDirAt 0.0f 0.0f
                    ToEnemy = enemyAimDirAt 0.0f 0.0f
                }
        }
    ...
    let runFramesOf (doc: Bulletml) (frames: int) : int = runPreparedApi (prepareApi doc) frames

    /// 読む段まで入る。走行だけを測る側では使わない。
    let runFrames (xml: string) (frames: int) : int = runFramesOf (parseXml xml) frames
    ...
    let countLiveDead (doc: Bulletml) (frames: int) : int * int =
        ...
        for _ in 0 .. frames - 1 do
            ...
            for i in 0 .. count - 1 do
                ...
                if bo.Used then
                    if finished.Contains it.Bullet then
                        deadCalls <- deadCalls + 1
                    else
