...
/// **二度 更新してある。** 400e425（aim skip を戻した版、比は
/// move -21.6 / 5way +4.3 / 10Way +4.1 / homing -3.2）で目盛りを合わせ、
/// 箱を 4 つ 値型にした手で比が 17 ポイント 動いたので d4361e7
/// （move -25.6 / 5way -13.0 / 10Way -12.4 / homing -4.9）で測り直し、
/// Sim の 3 手（Emit を voption / 包みを struct / bind と CE を inline）で
/// また動いたので、いまの値へ測り直した。
///
/// **口が軸の上に乗っていることは 2 回 確かめている。** どちらも、この口が
...
/// 2 回め の 0.1 ポイント は、バイナリが変わると動く 4,120 B の分
/// （allocOf の但し書き）。
let private calibratedAt = "bd40cd7（Sim の bind と CE ビルダを inline にした版）"

let private calibration = dict [ "move", -30.4; "5way", -14.0; "10Way", -13.4; "homing", -7.1 ]
