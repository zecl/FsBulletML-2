...
var repoRoot = args.Length > 0 ? args[0] : @"C:\Code\FsBulletML-2";
var proj = Path.Combine(repoRoot, @"samples\FsBulletML2.Sample.Unity2D.FSharp");

var verLine = File.ReadLines(Path.Combine(proj, @"ProjectSettings\ProjectVersion.txt")).First();
var ver = verLine.Replace("m_EditorVersion:", "").Trim();
var editor = $@"C:\Program Files\Unity\Hub\Editor\{ver}\Editor\Data\Managed";
...
if (Directory.Exists(pc))
    foreach (var d in Directory.GetDirectories(pc, "*", SearchOption.AllDirectories))
        if (d.EndsWith(@"\Runtime") || d.EndsWith(@"\lib")) AddDir(d);
