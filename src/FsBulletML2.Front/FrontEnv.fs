...
[<Sealed>]
type NearestEnemy<'E>(enemies: Func<IReadOnlyList<'E>>, ex: Func<'E, float32>, ey: Func<'E, float32>) =
...
module FrontEnv =
    ...
    let at (front: IFrontEnv) (space: Space) (origin: SpawnOrigin) (x: float32) (y: float32) : Env =
        ...
        let toEnemy =
            if front.TryTargetFrom(x, y, &tx, &ty) then
                Aiming.toward space x y tx ty
            else
                0.0f
        ...
        let spawnToEnemy =
            if front.TrySpawnTargetFrom(sx, sy, &stx, &sty) then
                Aiming.toward space sx sy stx sty
            else
                0.0f

        {
            ...
            Aim =
                {
                    ToPlayer = Aiming.toward space x y front.PlayerX front.PlayerY
                    ToEnemy = toEnemy
                }
            Spawn =
                {
                    ToPlayer = Aiming.toward space sx sy front.PlayerX front.PlayerY
