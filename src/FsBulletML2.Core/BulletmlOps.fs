...
module internal BulletmlOps =
    ...
    let private collect
        ...
        let rec command (c: Action) =
            ...
            | Action.Fire(attrs, d, s, child) -> fromFire (attrs, d, s, child) @ bulletElm child
        ...
        let topElm (t: BulletmlElm) =
            ...
            | BulletmlElm.Bullet(attrs, d, s, children) ->
                fromBullet (attrs, d, s, children) @ (children |> List.collect actionElm)
            | BulletmlElm.Fire(attrs, d, s, child) -> fromFire (attrs, d, s, child) @ bulletElm child
    ...
    let internal getBullet (bulletml: Bulletml) : BulletElm list =
        bulletml
        |> collect (fun _ _ -> []) (fun _ -> []) (fun (attrs, d, s, children) ->
    ...
    let rec private substCommandCore prams (c: Action) : Action =
        ...
        | Action.ChangeSpeed(speed, term) -> Action.ChangeSpeed(convertSpeed prams speed, convertTerm prams term)
        | Action.Accel(horizontal, vertical, term) ->
            Action.Accel(
                convertHorizontalOption prams horizontal,
                convertVerticalOption prams vertical,
                convertTerm prams term
            )
        | Action.Wait s -> Action.Wait(convertWait prams s)
        | Action.Vanish -> Action.Vanish
        | Action.Repeat(times, child) -> Action.Repeat(convertTimes prams times, substActionElm prams child)
        | Action.Fire(attrs, direction, speed, child) ->
            Action.Fire(
                attrs,
                convertDirectionOption prams direction,
                convertSpeedOption prams speed,
                substBulletElm prams child
            )
        | Action.FireRef(attrs, param) -> Action.FireRef(attrs, convertParam prams param)
        | Action.Action(attrs, children) -> Action.Action(attrs, children |> List.map (substCommand prams))
    ...
    and private substActionElmCore prams (a: ActionElm) : ActionElm =
        match a with
        | ActionElm.Action(attrs, children) -> ActionElm.Action(attrs, children |> List.map (substCommand prams))
    ...
    let internal refAction (target: ActionElm) (label: ActionLabel) prams : ActionElm =
        ...
        match target with
        | ActionElm.Action(attrs, _) when attrs.actionLabel = Some label -> substActionElm prams target
        | _ -> target
    ...
    let internal refFire (target: Action) (label: FireLabel) prams : Action =
        ...
        match target with
        | Action.Fire(attrs, _, _, _) when attrs.fireLabel = Some label -> substCommand prams target
        | _ -> target
    ...
    let internal refBullet (target: BulletElm) (label: BulletLabel) prams : BulletElm =
        ...
        match target with
        | BulletElm.Bullet(attrs, _, _, _) when attrs.bulletLabel = Some label -> substBulletElm prams target
        | _ -> target

    /// 参照を解く。輪を残してよいのは直近の action 自身へ戻るときだけ。別経由は呼び出しが毎フレーム深くなる。
    let rec private resolveActionRef visiting lastAction top (attrs: ActionRefAttrs) prams : ActionElm option =
        // None は「輪なので、そのまま残す」
        ...
        if Set.contains key visiting then
            ...
            | None ->
                new BulletmlDTDViolationException(
                    sprintf "not found target Action element:%s" (ActionLabel.text attrs.actionRefLabel)
                )
                |> raise
    ...
    and private expandCommandCore visiting lastAction top (c: Action) : Action =
        ...
        | Action.FireRef(attrs, prams) ->
            ...
            | None ->
                new BulletmlDTDViolationException(
                    sprintf "not found target Fire element:%s" (FireLabel.text attrs.fireRefLabel)
                )
                |> raise
        | Action.Action(attrs, children) ->
            Action.Action(attrs, children |> List.map (expandCommand visiting lastAction top))
        | Action.Fire(attrs, d, s, child) -> Action.Fire(attrs, d, s, expandBulletElm visiting lastAction top child)
        | Action.Repeat(times, child) -> Action.Repeat(times, expandActionElm visiting lastAction top child)
        | Action.ChangeDirection _
        | Action.ChangeSpeed _
        | Action.Accel _
        | Action.Wait _
    ...
    and private expandBulletElm visiting lastAction top (b: BulletElm) : BulletElm =
        ...
        | BulletElm.BulletRef(attrs, prams) ->
            ...
            if Set.contains key visiting then
                ...
                | None ->
                    new BulletmlDTDViolationException(
                        sprintf "not foun target Bullet element:%s" (BulletLabel.text attrs.bulletRefLabel)
                    )
                    |> raise
