...
module Sim =
    ...
    let internal get: Sim<BulletState> =
        fun _ st ->
            {
                Value = st
                State = st
...
type internal SimBuilder() =
    ...
    member inline _.Combine(a: Sim<unit>, [<InlineIfLambda>] b: unit -> Sim<'a>) = Sim.bind (fun () -> b ()) a
    ...
    member this.While(guard, body) =
        if guard () then
            Sim.bind (fun () -> this.While(guard, body)) (body ())
        else
            Sim.ret ()
