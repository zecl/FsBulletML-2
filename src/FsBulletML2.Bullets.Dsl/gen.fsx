...
let ex (e: Expr.NumExpr) = sprintf "numExpr %s" (q (Expr.NumExpr.text e))
...
let duList (items: string list list) : string list =
  ...
  else [ "[" ] @ (items |> List.collect ind) @ [ "]" ]

let rec duCommand (c: Action) : string list =
  match c with
  | Action.Wait e -> [ sprintf "Action.Wait (%s)" (ex e) ]
  | Action.Vanish -> [ "Action.Vanish" ]
  | Action.ChangeDirection (d, Term t) ->
    [ sprintf "Action.ChangeDirection (%s, Term (%s))" (duDirection d) (ex t) ]
  | Action.ChangeSpeed (s, Term t) ->
    [ sprintf "Action.ChangeSpeed (%s, Term (%s))" (duSpeed s) (ex t) ]
  ...
  | Action.Repeat (Times e, child) ->
    [ sprintf "Action.Repeat (Times (%s)," (ex e) ] @ ind (duActionElm child) @ [ ")" ]
...
let duBulletml (bml: Bulletml) : string list =
  match bml with
  | Bulletml (attrs, elms) ->
    let optQ (o: string option) = match o with Some s -> sprintf "Some %s" (q s) | None -> "None"
    let typ =
    ...
    [ "Bulletml"
      sprintf "  ({bulletmlXmlns = %s; bulletmlType = %s; bulletmlName = %s; bulletmlDescription = %s},"
        (optQ attrs.bulletmlXmlns) typ (optQ attrs.bulletmlName) (optQ attrs.bulletmlDescription) ]
...
let generateDu (p: Parsed) =
  ...
  for h in duHeader do w h
  w ""
  w ("namespace " + p.Namespace.Replace("FsBulletML2.Bullets.Dsl", "FsBulletML2.Bullets"))
  w "open FsBulletML2"
  w ""
  ...
  for e in p.Entries do
    ...
    if prefix then w "    createBulletmlInfo <|"
