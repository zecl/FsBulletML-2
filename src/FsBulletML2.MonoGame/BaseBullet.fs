...
type BaseBullet() as this =
    ...
    [<DefaultValue>]
    val mutable private self: IBullet
    ...
    interface IBullet with
            ...
            run <-
                ...
                |> Option.map (fun sc ->
                    if self.IsBullet then
                        Runner.newShot self.BulletType sc
                    else
                        Runner.newRoot self.BulletType sc)
        ...
        member this.Init() = this.self.Used <- true
    ...
    member this.RunTask(apply: Action<_, _>) =
            ...
            let motion: Motion =
                {
                    ...
                    Accel =
                        {
                            X = this.self.AccelerationX
                            Y = this.self.AccelerationY
                        }
                }
        ...
        if
            (this.pos.X < 0.f
             || this.pos.X > Settings.Display.Width
             || this.pos.Y < 0.f
             || this.pos.Y > Settings.Display.Height)
        then
            this.self.Used <- false
