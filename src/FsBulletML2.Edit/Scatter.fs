    ...
    Action.Fire(
        ...
        Some(
            Direction(
                ...
                Expr.NumExpr.ofString (sprintf "%.2f" deg)
            )
        ),
...
let private stemFire (stem: string) =
    fireAt 180.0 (DROP / float STEM_TERM) stem
...
let private stemBullet (stem: string) (seed: string) (count: int) =
    ...
    BulletmlElm.Bullet(
        { bulletLabel = Some(BulletLabel stem) },
        None,
        None,
        [
            ActionElm.Action(
...
let private seedBullet (name: string) (wave: Action list) =
    BulletmlElm.Bullet(
        { bulletLabel = Some(BulletLabel name) },
        None,
        None,
        [
            ActionElm.Action(
    ...
    let rec one (a: Action) : Action list option =
        ...
        | Action.ChangeSpeed(_, Term t) ->
            if ok t then
                Some
                    [
                        Action.ChangeSpeed(Speed(Some { speedType = SpeedType.Relative }, zero), Term t)
                    ]
            else
                None
        ...
        | Action.Accel _
        | Action.Fire _
        | Action.FireRef _ -> Some []
        | Action.Vanish
        | Action.Repeat _ -> None
    ...
    and refOf (attrs: ActionRefAttrs) (ps: Params) : Action list option =
        ...
        if List.contains l seen then
            ...
            | Some body ->
                ...
                let filledBody =
                    if Map.isEmpty param then
                        body
                    else
                        List.map (fillIn param) body
