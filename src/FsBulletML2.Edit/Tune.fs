...
[<Literal>]
let private STEP = 1.25

/// 掛けた 数 を丸める 桁。`1.25 * 0.8` が 1.0 に戻らない と、
/// 押し戻して も `(元) * 1.0000000000000002` が残る
[<Literal>]
let private DIGITS = 6

let private fmt (v: float) =
    let s =
        Math.Round(v, DIGITS).ToString("0.######", Globalization.CultureInfo.InvariantCulture)
...
let private scale (k: float) (e: Expr.NumExpr) : Expr.NumExpr =
    ...
    if v = 1.0 then
        Expr.NumExpr.ofString inner
    else
        Expr.NumExpr.ofString (sprintf "(%s) * %s" inner (fmt v))
...
let private spinnable (d: Direction) =
    match d with
    | Direction(Some a, _) ->
        a.directionType = DirectionType.Sequence
        || a.directionType = DirectionType.Relative
...
let private addLayer (bulletml: Bulletml) : Bulletml =
    ...
    | Bulletml(attrs, elms) ->
        ...
        | Some(BulletmlElm.Action(_, body)) when existing < MAX_LAYER ->
            ...
            let spun =
                if n % 2 = 0 then
                    List.map (mapAction Steadier >> mapAction Steadier) body
                else
                    body
                        ...
                        Action.Fire(
                            ...
                            Some(
                                Direction(
                                    Some
                                        {
                                            directionType = DirectionType.Sequence
                                        },
                                    num "90"
                                )
                            ),
...
and private dropDeepestAction (a: Action) : Action * bool =
    ...
    | Action.Action(attrs, xs) ->
        ...
        let ys =
            xs
            |> List.map (fun x ->
                let x', h = dropDeepestAction x in
                (hit <- hit || h)
                x')
...
let private dropSplit (bulletml: Bulletml) =
    ...
    | Bulletml(attrs, elms) ->
        let top (e: BulletmlElm) =
            ...
            | BulletmlElm.Action(a, xs) -> BulletmlElm.Action(a, xs |> List.map (fun x -> fst (dropDeepestAction x)))
...
[<Literal>]
let private HUSH_PREFIX = "tuned-hush"

[<Literal>]
let private MAX_HUSH = 3
...
and private hushInBullet target n d (b: BulletElm) =
    match b with
    | BulletElm.Bullet(attrs, dir, s, xs) -> BulletElm.Bullet(attrs, dir, s, List.map (hushInElm target n d) xs)
    | BulletElm.BulletRef _ -> b
...
let private hushTop target n (e: BulletmlElm) =
    ...
    | BulletmlElm.Bullet(attrs, d, s, xs) -> BulletmlElm.Bullet(attrs, d, s, List.map (hushInElm target n 0) xs)
    | BulletmlElm.Fire(attrs, d, s, b) -> BulletmlElm.Fire(attrs, d, s, hushInBullet target n 1 b)
...
let private hush (bulletml: Bulletml) =
    ...
    | Bulletml(attrs, elms) ->
        ...
        if target <= 1 || n > MAX_HUSH then
            bulletml
        else
            Bulletml(attrs, List.map (hushTop target n) elms)
...
let private unhush (bulletml: Bulletml) =
    ...
    if last = 0 then
        ...
        let strip (a: Action) =
            match a with
            | Action.Action({ actionLabel = Some(ActionLabel l) }, [ Action.Repeat(_, ActionElm.Action(_, [ inner ])) ]) when
                l = name
                ->
                inner
            | _ -> a
...
/// 足した 寿命 の印
[<Literal>]
let THIN_MARK = "tuned-thin"

/// 終点 の弾 が 消える まで の コマ。`Combine.LIFE` と同じ 数 で、口 が違う
[<Literal>]
let private THIN_LIFE = "60"
...
let private tweak (k: float) (e: ActionElm) : ActionElm option =
    ...
    | ActionElm.Action(attrs, xs) when isThin e ->
        let bare =
            ...
            |> List.exists (fun a ->
                match a with
                | Action.Wait w -> Expr.NumExpr.text w = THIN_LIFE
                | _ -> false)
        ...
        if bare && k > 1.0 then
            ...
            let ys =
                ...
                |> List.map (fun a ->
                    match a with
                    | Action.Wait w -> Action.Wait(scale k w)
                    | _ -> a)
