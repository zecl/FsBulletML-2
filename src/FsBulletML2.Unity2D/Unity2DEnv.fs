...
/// このフロントが `FsBulletML2.Front` の口に答えるところ。式は `Aiming.toward` の `Space` 違い。
/// 弾 1 個につき 1 個。使い回すと別の弾の相手を引き継ぐ。
[<Sealed>]
type Unity2DEnv() =

    let near =
        NearestEnemy<IDefaultBullet>(
            (fun () -> Manager.enemies :> IReadOnlyList<IDefaultBullet>),
            (fun e -> e.X),
            (fun e -> e.Y)
        )
