...
type DefaultBullet (transform:Transform) =
  ...
  interface IDefaultBullet with 
    ...
    member this.Init () = 
      ...
      // ここでの self の位置は、この Init 呼び出しの時点の位置（fireCommand からの
      // 呼び出しでは、まだ親の位置へ移す前）。envOfGlobal はその時点の self から組む
      self.Task |> function
      | None -> ()
      | Some x -> x.Init(BulletRunner.envOfGlobal self)
  ...
  member this.RunTask(apply:Action<_,_>) =
    ...
    | Some task -> 
      ...
      if result.Processed then
        ...
        // 位置を更新したあとの self から組む（envOfGlobal は位置を読むので、
        // apply より前の値を使い回すと aim がずれる）
        self.Task |> Option.iter (fun x -> x.Init(BulletRunner.envOfGlobal self))
