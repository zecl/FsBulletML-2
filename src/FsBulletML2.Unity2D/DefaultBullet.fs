    ...
    interface IDefaultBullet with
        ...
        member _.SetScript(s) =
            ...
            run <-
                ...
                |> Option.map (fun sc ->
                    if me.IsBullet then
                        Runner.newShot me.BulletType sc
                    else
                        Runner.newRoot me.BulletType sc)
        ...
        member this.Init() =
            ...
            // 組むところも旧のまま
            run <- run |> Option.map (fun r -> Driver.restart front r)
