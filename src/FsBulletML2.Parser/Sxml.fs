...
module Sxml =
    ...
    let pAttr =
        ...
        skipSpaces1 .>> pFollowed >>. pLabel .>>. pVal
    ...
    type private SxmlSink() =
        ...
        let check (where: string) (illegal: string) (value: string) =
            let hit =
                value
                |> Seq.filter (fun c -> illegal.IndexOf c >= 0)
                |> Seq.distinct
                |> Seq.toList
        ...
        interface IBulletmlSink with
            ...
            member _.Attr(name, value) =
                ...
                sb.Append(" (").Append(name).Append(" \"").Append(check name illegalInAttr value).Append("\")")
                |> ignore
