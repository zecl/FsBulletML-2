...
module Offside =
    ...
    let pAttrsAndBody =
        parse {
            let pBodyValue =
                chr '"' >>. manyChars (asciiLetter <|> digit <|> anyOf "()$+-*/.%") .>> chr '"'
    ...
    let pOpenIndent =
        parse {
            ...
            do!
                (UserState.UpdateDepth(state.Current :: state.Depth)
                 >> UserState.UpdateCurrent(state.Next))
                |> updateUserState
        }
    ...
    let pCloseIndent =
        parse {
            ...
            do!
                (UserState.UpdateDepth(List.tail state.Depth)
                 >> UserState.UpdateCurrent(List.head state.Depth))
                |> updateUserState
        }
    ...
    [<CompiledName "Parse">]
    let parse input =
        runParserOnString pAst (UserState.Create()) "" input
    ...
    type private FsbSink() =
        ...
        interface IBulletmlSink with
            ...
            member _.Text value =
                let squashed =
                    System.String(value.ToCharArray() |> Array.filter (fun c -> not (System.Char.IsWhiteSpace c)))

                if squashed = "" then
                    // 空の本文を書くと、読み直したときに子が 1 つ も無い形になって消える
