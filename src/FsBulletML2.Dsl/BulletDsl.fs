...
module Dsl =
    ...
    let private emptyBullet = BulletElm.Bullet({ bulletLabel = None }, None, None, [])

    let private wrapAction (label: string option) (cmds: Action list) =
        ActionElm.Action(
            {
                actionLabel = label |> Option.map ActionLabel
            },
            cmds
        )
    ...
    let changeDirectionAbs s term =
        changeDirOf DirectionType.Absolute s term

    let changeDirectionRel s term =
        changeDirOf DirectionType.Relative s term

    let changeDirectionSeq s term =
        changeDirOf DirectionType.Sequence s term
    ...
    type FireBuilder<'T>(label: string option, finish: FireAttrs * Direction option * Speed option * BulletElm -> 'T) =
        ...
        [<CustomOperation("dir")>]
        member _.Dir(s: FireSpec, e) =
            { s with
                Dir = Some(Direction(None, expr e))
            }

        [<CustomOperation("aim")>]
        member _.Aim(s: FireSpec, e) =
            { s with Dir = dir DirectionType.Aim e }

        [<CustomOperation("absolute")>]
        member _.Absolute(s: FireSpec, e) =
            { s with
                Dir = dir DirectionType.Absolute e
            }

        [<CustomOperation("relative")>]
        member _.Relative(s: FireSpec, e) =
            { s with
                Dir = dir DirectionType.Relative e
            }

        [<CustomOperation("sequence")>]
        member _.Sequence(s: FireSpec, e) =
            { s with
                Dir = dir DirectionType.Sequence e
            }
        ...
        member _.SpeedAbs(s: FireSpec, e) =
            { s with
                Speed = spd SpeedType.Absolute e
            }

        [<CustomOperation("speedRel")>]
        member _.SpeedRel(s: FireSpec, e) =
            { s with
                Speed = spd SpeedType.Relative e
            }

        [<CustomOperation("speedSeq")>]
        member _.SpeedSeq(s: FireSpec, e) =
            { s with
                Speed = spd SpeedType.Sequence e
            }
        ...
        member _.RefBullet(s: FireSpec, label: string, ps: string list) =
            { s with
                Body = Some(BulletElm.BulletRef({ bulletRefLabel = BulletLabel label }, ps))
            }
    ...
    let topFireAs name =
        FireBuilder(Some name, BulletmlElm.Fire)
    ...
    type AccelBuilder(term: string) =
        ...
        member _.Run(s: AccelSpec) = Action.Accel(s.H, s.V, Term(expr term))
                ...
                H =
                    Some(
                        Horizontal(
                            Some
                                {
                                    horizontalType = HorizontalType.Absolute
                                },
                            expr e
                        )
                    )
                ...
                H =
                    Some(
                        Horizontal(
                            Some
                                {
                                    horizontalType = HorizontalType.Relative
                                },
                            expr e
                        )
                    )
                ...
                H =
                    Some(
                        Horizontal(
                            Some
                                {
                                    horizontalType = HorizontalType.Sequence
                                },
                            expr e
                        )
                    )
        ...
        member _.Vertical(s: AccelSpec, e) =
            { s with
                V = Some(Vertical(None, expr e))
            }

        [<CustomOperation("verticalAbs")>]
        member _.VerticalAbs(s: AccelSpec, e) =
            { s with
                V = Some(Vertical(Some { verticalType = VerticalType.Absolute }, expr e))
            }

        [<CustomOperation("verticalRel")>]
        member _.VerticalRel(s: AccelSpec, e) =
            { s with
                V = Some(Vertical(Some { verticalType = VerticalType.Relative }, expr e))
            }

        [<CustomOperation("verticalSeq")>]
        member _.VerticalSeq(s: AccelSpec, e) =
            { s with
                V = Some(Vertical(Some { verticalType = VerticalType.Sequence }, expr e))
            }
    ...
    type BulletBuilder<'T>
        (label: string option, finish: BulletAttrs * Direction option * Speed option * ActionElm list -> 'T) =
        ...
        [<CustomOperation("aim")>]
        member _.Aim(s: BulletSpec, e) =
            { s with Dir = dir DirectionType.Aim e }

        [<CustomOperation("absolute")>]
        member _.Absolute(s: BulletSpec, e) =
            { s with
                Dir = dir DirectionType.Absolute e
            }

        [<CustomOperation("relative")>]
        member _.Relative(s: BulletSpec, e) =
            { s with
                Dir = dir DirectionType.Relative e
            }

        [<CustomOperation("sequence")>]
        member _.Sequence(s: BulletSpec, e) =
            { s with
                Dir = dir DirectionType.Sequence e
            }
        ...
        [<CustomOperation("speedAbs")>]
        member _.SpeedAbs(s: BulletSpec, e) =
            { s with
                Speed = spd SpeedType.Absolute e
            }

        [<CustomOperation("speedRel")>]
        member _.SpeedRel(s: BulletSpec, e) =
            { s with
                Speed = spd SpeedType.Relative e
            }

        [<CustomOperation("speedSeq")>]
        member _.SpeedSeq(s: BulletSpec, e) =
            { s with
                Speed = spd SpeedType.Sequence e
            }
        ...
        member _.RefActs(s: BulletSpec, label: string, ps: string list) =
            { s with
                Actions = s.Actions @ [ bodyRef label ps ]
            }

    let bullet name =
        BulletBuilder(Some name, BulletElm.Bullet)

    let bulletAnon = BulletBuilder(None, BulletElm.Bullet)

    let defBullet name =
        BulletBuilder(Some name, BulletmlElm.Bullet)
    ...
    let vertical name =
        BulletmlBuilder(Some ShootingDirection.BulletVertical, Some name, None, None)

    let horizontal name =
        BulletmlBuilder(Some ShootingDirection.BulletHorizontal, Some name, None, None)

    let none name =
        BulletmlBuilder(Some ShootingDirection.BulletNone, Some name, None, None)

    /// type 属性を書かない bulletml。DTD の既定は none だが、値としては別物
    /// （書かないことを None で持つので、往復で type 属性が出なくなる）
    let untyped name =
        BulletmlBuilder(None, Some name, None, None)
    ...
    let untypedXmlns xmlns name =
        BulletmlBuilder(None, Some name, Some xmlns, None)

    /// 名前を書かない根。本家の弾幕はこちらが普通。
    /// `name` はこのエンジンが足した属性で、同梱は持っていない。
    let verticalAnon =
        BulletmlBuilder(Some ShootingDirection.BulletVertical, None, None, None)

    let horizontalAnon =
        BulletmlBuilder(Some ShootingDirection.BulletHorizontal, None, None, None)
    ...
    let noneXmlnsAnon xmlns =
        BulletmlBuilder(Some ShootingDirection.BulletNone, None, Some xmlns, None)

    let untypedXmlnsAnon xmlns =
        BulletmlBuilder(None, None, Some xmlns, None)
    ...
    module Examples =
        ...
        let attributeShapes =
            [
                untypedXmlns "http://www.asahi-net.or.jp/~cs8k-cyu/bulletml" "type なし + xmlns" { top { vanish } }
                horizontalXmlns "http://www.asahi-net.or.jp/~cs8k-cyu/bulletml" "horizontal + xmlns" { top { vanish } }
                noneXmlns "http://www.asahi-net.or.jp/~cs8k-cyu/bulletml" "none + xmlns" { top { vanish } }
                // name を書かない形と description を入れる形は、短い入口を置いていない
                bulletmlOf (Some ShootingDirection.BulletVertical) None None None { top { vanish } }
                bulletmlOf None (Some "説明つき") None (Some "description は BulletML 公式の属性ではない") { top { vanish } }
            ]
        ...
        let allTypeVariants =
            vertical "型の全通り" {
                ...
                defBullet "b" { doActs (bodyRef "top" []) }
            }
        ...
        let bulletShapes =
            vertical "bullet の形" {
                ...
                defBullet "labelled" { doActs (bodyAs "inner" { wait "1" }) }
