...
module HarmonicSpec =
    ...
    let create (f: HarmonicAxes -> HarmonicAxes) : HarmonicSpec =
        ...
        let folds2, amp2, layers0, layerPhase, layerScale =
            if a.Seed <= 0 then
                a.Folds2, a.Amplitude2, a.Layers, a.LayerPhase, a.LayerScale
            else
                ...
                (if a.Layers > 0 then a.Layers
                 elif a.Figure = Heart then 1
                 else l),
...

        {
            Axes_ =
                { a with
                    ...
                    Arms =
                        (let cap = max 8 (160 / blooms)

                         let arms =
                             if a.Arms = 0 then
                                 min cap (max 16 (folds * 24 / blooms))
                             else
                                 max 8 (min cap a.Arms)

                         let n = symmetry a.Figure folds folds2

                         if layers > 1 && n >= 2 && arms >= 2 * n then
                             arms - arms % n
                         else
                             arms)
                    Blooms = blooms
...
module Harmonic =
    ...
    let private symmetryOf (h: HarmonicSpec) =
        HarmonicSpec.symmetry h.Figure h.Folds h.Folds2
    ...
    let private layerShift (h: HarmonicSpec) (j: int) =
        if j = 0 then
            ...
            float j * h.LayerPhase * 2.0 * Math.PI / float (symmetryOf h)
    ...
    let private starMean =
        ...
        |> List.map (fun k ->
            k,
            (Seq.init n (fun i -> starAt (float k) (float i * 2.0 * Math.PI / float n))
             |> Seq.average))
        |> Map.ofList
    ...
    let private speedAt (h: HarmonicSpec) (j: int) (t0: float) =
        ...
        let r =
            if s > 0.0 then
                r0 h * (1.0 + float j * s) / (1.0 + float (h.Layers - 1) * s)
            else
                r0 h
        // 入れ子 で 内 と 外 が 交わらない のは a < s / (2 + s)。その 9 割 で 頭打ち に する
        let a =
            if s > 0.0 then
                min (amp h / r0 h) (0.9 * s / (2.0 + s))
            else
                amp h / r
    ...
    let private speedOf (spd: float) =
        sprintf "%.2f + $rank * %.2f" spd (spd * rankFactor)

    /// 弾 が毎コマ 曲がる 角（度）。1 輪 が同じ 角 で曲がる ので 花 は形 を保った まま ω/2 で回る。
    /// 曲げる ほど 広がり が縮む ので 1 度 で止める
    let private turnDeg (h: HarmonicSpec) =
        if h.Spin > 0.0 then 0.5 + 0.25 * h.Spin else 0.0

    /// 曲がる の は ここ まで。曲がり 続ける と 遅い 弾 は半径 v / ω の円 を描いて 出て行かない
    [<Literal>]
    let private TURN_TERM = 120
    ...
    let private foldBy (h: HarmonicSpec) =
        ...
        if n >= 2 && h.Arms % n = 0 && h.Arms / n >= 2 then
            Some(h.Arms / n)
        else
            None
    ...
    let private ring (h: HarmonicSpec) : Action list =
        ...
        let step i =
            withLayers i (shot (sprintf "%.2f" gap) (snd (armOf h 0 i)))
    ...
    let aliveBound (h: HarmonicSpec) : float =
        ...
        let perWave =
            (shots |> List.sumBy (fun (i, j) -> lifeOf (snd (armOf h j i))))
            * float h.Blooms
    ...
    let fitTo (budget: float) (h: HarmonicSpec) : HarmonicSpec =
        ...
        let rec go (s: HarmonicSpec) (tries: int) =
            ...
            if need <= 1.0 || tries <= 0 then
                s
            else
                go (HarmonicSpec.withWaitScale (s.WaitScale * max 2.0 (ceil need)) s) (tries - 1)
