...
module LineageSpec =
    ...
    let private waves (s: LineageSpec) =
        if s.Alternate && s.Root <> Root.Bar then
            2.0 * floor ((at s.Waves + 1.0) / 2.0)
        else
            at s.Waves
    ...
    let private cycle (s: LineageSpec) =
        match s.Root with
        | Root.Radial
        | Root.Fan -> waves s * at1 s.WaveWait
    ...
    let internal stageSpeed (s: LineageSpec) (i: int) =
        if i = 0 then
            (match s.Root with
             | Root.Radial
             | Root.Fan -> s.RootSpeed
             | Root.Bar -> BAR_LINE_SPEED)
        else
            (match s.Chain.[i - 1] with
             | Spawner.Trail -> CHILD_SPEED
             | Spawner.Burst -> BURST_SPEED)

    /// 段 `i` が 止まる まで に 飛ぶ コマ数（`$rank = 1` の 速さ で 数える）
    let flyOf (s: LineageSpec) (i: int) =
        let stops =
            (s.Chain |> List.filter ((=) Spawner.Burst) |> List.length)
            + (if s.Leaf = Terminal.Relaunch then 1 else 0)
    ...
    let private lifeOf (s: LineageSpec) (i: int) =
        ...
        if i < s.Chain.Length then
            ...
            let relaunch =
                ...
                + at1 s.RelaunchHold
                + float RELAUNCH_TERM
                + FIELD_SPAN / fast RELAUNCH_SPEED
    ...
    let private shrink (s: LineageSpec) : LineageSpec option =
        ...
        if s.WaveWait.Base < 360 then
            Some
                { s with
                    WaveWait =
                        {
                            Base = s.WaveWait.Base * 2
                            Rank = s.WaveWait.Rank * 2
                        }
                }
        elif trail && s.TrailTimes > 4 then
            Some { s with TrailTimes = s.TrailTimes / 2 }
        elif s.Root = Root.Bar && s.BarSteps.Base > 2 then
            Some
                { s with
                    BarSteps =
                        {
                            Base = s.BarSteps.Base / 2
                            Rank = s.BarSteps.Rank / 2
                        }
                }
        elif s.Line.Rank > 0 then
            Some
                { s with
                    Line = { s.Line with Rank = s.Line.Rank - 1 }
                }
        elif s.Line.Base > 1 then
            Some
                { s with
                    Line = { s.Line with Base = s.Line.Base - 1 }
                }
        elif s.Columns = 2 then
            Some { s with Columns = 1 }
        elif trail && s.TrailTimes > 2 then
            Some { s with TrailTimes = s.TrailTimes / 2 }
        elif s.WaveWait.Base < 1440 then
            Some
                { s with
                    WaveWait =
                        {
    ...
    let rec private fit (s: LineageSpec) (tries: int) =
        if tries <= 0 || aliveBound s <= BUDGET then
            s
        else
            match shrink s with
    ...
    let create (f: LineageAxes -> LineageAxes) : LineageSpec =
        ...
        let chain =
            if a.Seed > 0 then
                CHAIN_TABLE.[(a.Seed - 1) % 9]
            else
                let g = max 1 (min 3 a.Generations)

                if streak >= 1.0 then
                    Spawner.Trail :: List.replicate (g - 1) Spawner.Burst
                else
                    List.replicate g Spawner.Burst
        ...
        let s: LineageSpec =
            {
                ...
                Waves =
                    (match a.Root with
                     | Root.Radial
                     | Root.Fan -> { Base = 1; Rank = 1 }
                     | Root.Bar -> { Base = 1; Rank = 0 })
        ...
        let s =
            { s with
                FallHold =
                    if leaf = Terminal.Fall then
                        fallHoldOf leafSpeed s.Gravity
                    else
                        0
            }
        ...
        fit
            { s with
                TrailTimes = min MAX_REPEAT (max 1 screen)
            }
            30
...
module Lineage =
    ...
    let private speedOf (v: float) =
        sprintf "%.2f + $rank * %.2f" v (v * 0.3)
    ...
    let private childOf (s: LineageSpec) (i: int) =
        if i + 1 < s.Chain.Length then label (i + 1) else "leaf"
    ...
    let private trailBody (s: LineageSpec) (child: string) =
        ...
        let plainPair (c: string) =
            [
                fire {
                    relative "90"
                    speed (speedOf LineageSpec.CHILD_SPEED)
                    refBullet c args
                }
                fire {
                    relative "-90"
        ...
        if s.Sweep = 0.0 && s.Strands = 1 then
            ...
            let d =
                if s.TrailTimes > 1 then
                    s.Sweep / float (s.TrailTimes - 1)
                else
                    0.0
            ...
            let stepOf =
                if muls.Length > 1 then
                    speedOf (LineageSpec.CHILD_SPEED * (muls.[1] - muls.[0]))
                else
                    ""
            // 狙い 直す 弾 は 止まって 自機 へ 向き 直す ので、撚って も 同じ 所 に 集まる。1 本 だけ 撃つ
            let side (head: Action) (c: string) =
                if muls.Length = 1 || c = "seeker" then
                    [ head ]
                else
                    [
                        head
                        repeat (string (muls.Length - 1)) {
                            fire {
                                sequence "0"
                                speedSeq stepOf
                                refBullet c args
    ...
    let private burstBody (s: LineageSpec) (i: int) (child: string) =
        ...
        body {
            ...
            fire {
                relative "90"
                speed (speedOf LineageSpec.BURST_SPEED)
                refBullet head (passOn s)
            }
    ...
    let private argsAt (s: LineageSpec) (i: int) (w: int) =
        if s.Alternate then [ string (sign i w) ] else []

    let private pairsOf (r: Ranked) =
        sprintf "(%d + %d * $rank + 1) / 2" r.Base r.Rank
    ...
    let private waveBody (s: LineageSpec) =
        ...
        let shot (odd: bool) =
            fire {
                sequence (sprintf "%.2f" step)
                speed (speedOf s.RootSpeed)
                refBullet "g1" (args odd)
            }
        ...
        [
            yield
                fire {
                    aim (sprintf "%.2f" first)
                    speed (speedOf s.RootSpeed)
                    refBullet "g1" (args false)
                }
            if more >= 2 then
                yield repeat (string (more / 2)) { yield! [ shot true; shot false ] }
            if more % 2 = 1 then
                yield shot true
    ...
    let private spreadTop (s: LineageSpec) =
        top {
            if s.Alternate then
                repeat (pairsOf s.Waves) { yield! [ actionRef "wave" [ "1" ]; actionRef "wave" [ "-1" ] ] }
            else
        ...
        let pads (w: int) =
            if s.Alternate then
                [
                    for i in 0 .. s.Ways - 1 ->
                        fire {
                            aim (sprintf "%.2f" (gap * float i))
                            speed STOP
                            refBullet "bar" (argsAt s i w)
                        }
                ]
                @ [ wait (barCycle s) ]
            else
        ...
        let line (head: Action) =
            [
                ...
                repeat (timesOf s.Line) {
                    fire {
                        sequence "0"
                        speedSeq (sprintf "%.2f" BAR_LINE_STEP)
                        refBullet "g1" (passOn s)
                    }
                }
            ]
                ...
                body {
                    yield!
                        line (
                            fire {
                                relative "0"
                                speed (speedOf LineageSpec.BAR_LINE_SPEED)
                                refBullet "g1" (passOn s)
                            }
                        )
    ...
    let generate (s: LineageSpec) : BulletmlInfo =
        ...
        <| vertical "lineage" {
            ...
            if s.Seekers then
                defBullet "seeker" { doActs (seekerBody s) }
        }
