...
let bandScore (s: Snapshot) : float =
    ...
    if groups.IsEmpty then
        ...
        if vx + vy < 1e-9 then 0.0 else 1.0 - min 1.0 (vy / vx)
...
let ringScore (s: Snapshot) : float =
    if s.Headings.Length < 2 then
        0.0
    else
        min 1.0 (twoPi / float s.Headings.Length / maxGapOf s.Headings)
...
let fanScore (s: Snapshot) : float =
    if s.Headings.IsEmpty then
        0.0
    elif spanOf s.Headings >= System.Math.PI then
        0.0
    else
...
let fitScore (k: int) (f: float -> float) (s: Snapshot) : float =
    ...
    if pts.Length < 3 then
        ...
        if sst < 1e-12 then
            ...
            let row (t: float) =
                [| 1.0; sin (float k * t); cos (float k * t) |]

            let rows = pts |> Array.map (fst >> row)
            let a = Array2D.init 3 3 (fun i j -> rows |> Array.sumBy (fun r -> r.[i] * r.[j]))

            let b =
                Array.init 3 (fun i -> Array.fold2 (fun acc (r: float[]) y -> acc + r.[i] * y) 0.0 rows ys)
...
module Draw =
    ...
    let fullest (snaps: Snapshot list) : Snapshot =
    ...
    let private cell (ox: float) (oy: float) (label: string) (s: Snapshot) =
        ...
        let dots =
            pts
            |> List.map (fun (x, y) ->
                sprintf "<circle cx=\"%.1f\" cy=\"%.1f\" r=\"1.6\"/>" (ox + c + x * k) (oy + c + y * k))
            |> String.concat ""
