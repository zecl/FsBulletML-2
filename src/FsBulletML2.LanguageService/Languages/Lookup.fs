...
let hover (v: Vocab) (title: Token -> string) (token: Token) : string option =
    let element name =
        v.Elements |> List.tryFind (fun e -> e.Name = name)

    let attribute el at =
        element el
        |> Option.bind (fun e -> e.Attrs |> List.tryFind (fun a -> a.Name = at))
    ...
    | AttrValue(el, at, value) ->
        ...
        |> Option.bind (fun a ->
            ...
            |> Option.map (fun (_, spec) ->
                let spec =
                    if List.contains value a.Defaults then
                        spec + "\n\n**省いたときはこれ。**"
                    else
                        spec
...
let usages (v: Vocab) (tags: string -> TagHit list) (source: string) (token: Token) : Usage list =
    ...
    | AttrValue(element, attr, value) ->
        let related =
            ...
            |> List.filter (fun (refName, defName, a) -> a = attr && (refName = element || defName = element))
...
type VocabularyLanguage(shape: Shape, vocabulary: unit -> Vocab) =
    ...
    let find name =
        (vocabulary ()).Elements |> List.tryFind (fun e -> e.Name = name)
        ...
        | InContent(Some parent) ->
            ...
            | Some e ->
                ...
                if not e.Text then
                    ...
                    frames
                    @ children
                    @ ((vocabulary ()).Expressions |> List.map (Completion.plain exprLen))
        | InStartTag element ->
    ...
    member _.LensesIn(source: string) : Lens list = lenses (vocabulary ()) tagsOf source
    ...
    member _.HintsIn(source: string) : ExprSpot list =
        ...
        let places =
            Scan.lineColumnsAscending source (kept |> List.map (fun (_, _, stop) -> stop))
    ...
    member _.SignatureIn(source: string, offset: int) : Signature option =
        ...
        for i in 0 .. tags.Length - 1 do
            ...
            if
                not t.Closing
                && pairs |> List.exists (fun (refName, _, _) -> refName = t.TagName)
            then
                let mutable k = i + 1
        ...
        if found < 0 then
            ...
            let attrName =
                pairs
                |> List.pick (fun (refName, _, a) -> if refName = t.TagName then Some a else None)

            let label =
                t.Attrs
                |> List.tryPick (fun a -> if a.AttrName = attrName then Some a.Value else None)
            ...
            while k < tags.Length && tags.[k].Depth > t.Depth do
                ...
                if not u.Closing && List.contains u.TagName argNames && u.Depth = t.Depth + 1 then
                    ...
                    while m < tags.Length && tags.[m].Depth > u.Depth do
                        m <- m + 1

                    let uEnd = if m < tags.Length then tags.[m].Start else stop
                    given <- n :: given

                    if offset >= u.Start && offset <= uEnd then
                        active <- n
            ...
            let detail =
                if label = "" then
                    "名前がまだ無い"
                elif takes = 0 && n = 0 then
                    "この定義は引数を使わない"
                elif takes = n then
                    string takes + " つ 取って、" + string n + " つ 渡している"
                else
                    string takes + " つ 使っているが、" + string n + " つ しか渡していない"
                    |> fun s ->
                        if n > takes then
                            string takes + " つ 使っているのに、" + string n + " つ 渡している"
                        else
                            s
