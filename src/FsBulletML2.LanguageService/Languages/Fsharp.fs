...
type FsharpLanguage(vocabulary: unit -> Vocab) =
    ...
    let definitionAt (v: Vocab) (source: string) (defName: string) (_attr: string) (value: string) =
        ...
        | Some c ->
            ...
            | Some at ->
                ...
                if Scan.blankBefore source at then
                    Some(Scan.lineStart source at, body)
                else
                    Some(at, "\n" + body)
    ...
    static let title (name: string) (token: Token) =
        ...
        | AttrValue(element, attr, value) -> name + " → <" + element + " " + attr + "=\"" + value + "\">"
        | Nothing -> name
                        ...
                        |> List.choose (fun c ->
                            let token =
                                if c.Attr = "" then
                                    ...
                                    AttrValue(c.Element, c.Attr, c.Value)
