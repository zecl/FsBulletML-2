...
let findings (pairs: (string * string * string) list) (topPrefix: string) (tags: TagHit list) : Finding list =
    ...
    let allDefs =
        [
            for (_, defName, attrName) in pairs do
                yield! namedOf defName attrName
        ]
    ...
    let entryPoints =
        if topPrefix = "" then
            ...
            allDefs |> List.filter (fun (_, a) -> a.Value.StartsWith topPrefix)
        ...
        [
            for (refName, defName, attrName) in pairs do
                ...
                for (_, a) in namedOf defName attrName do
                    ...
                    if
                        not (topPrefix <> "" && a.Value.StartsWith topPrefix)
                        && not (List.contains a.Value used)
                    then
                        yield
                            {
    ...
    noEntry
    @ (unused @ missing @ duplicates |> List.sortBy (fun f -> f.Line, f.Column))
