...
///
/// **字は器が組む。** `Finding` と違って呼ぶ側が組み直す材料が無い ——
/// 「3 か所 から参照」と「根から走る」は同じ列に並ぶ 2 通り の字で、
/// **どちらを出すかを決めているのが器の側**（`topPrefix` を知っている）。
type Lens =
  { /// 出す字
    Title: string
...
type ISourceLanguage =
  ...
  /// 本文ぜんぶ の話で、Monaco は見えている範囲を自分で選ぶ。
  ///
  /// **根から走る定義には「0 か所 から参照」と出さない。**
  /// 同梱 176 本 で数えると、参照が 0 の定義 215 件 のうち **209 件 が
  /// 根から走るもの** —— そのまま出すと 9 割 の本に嘘が並ぶ。
  ///
  ...
  ///
  /// **F# の CE は空**（式の取り出しが無いので。v4.1 と同じ）
  abstract Hints: source: string -> ExprSpot list
  /// 参照が渡す引数の形（v4.5）。**カーソルを見る。**
  ///
  /// `◯◯Ref` の中に居なければ `None`。**空を返さない** ——
  /// 空でも枠が浮くので、出ていないことと見分けがつかなくなる（`Hover` と同じ）。
  ///
  /// **数が食い違っていても警告にしない。** 同梱 176 本 に **9 件 在る**
  /// （版の頭で数えた）—— v2.3 が引いた線（正しい弾幕にも在るものは出さない）に
  /// 掛かる。出すのは形であって、正しさの判定ではない。
  ///
  /// **F# の CE は `None`**（`◯◯Ref` と打たないので、囲みが決められない）
  ...
  /// 木は字の位置を持たない（`XmlNode` は 名前・属性・子 だけ）。
  ///
  /// **F# の CE は空。** あちらは要素名で書かないので札が名前しか返さず、
  /// 入れ子も `{ }` の段で要素の入れ子とは別物（v2.4.5 で 176 本 中 145 本 割れた）。
  /// **「結べない」は測ってある**ので、推定で光らせない。
  ///
