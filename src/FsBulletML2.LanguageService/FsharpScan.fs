...
module FsharpScan =
    ...
    let wordAt (src: string) (offset: int) : string option =
        if isNull src || src.Length = 0 then
            None
        elif offset < 0 || offset >= src.Length then
            None
        else
            let n = src.Length
            let at = offset
            ...
            if not (isIdent src.[at]) || not ok.[at] then
                None
            else
                let mutable s = at
    ...
    let describe (src: string) (cursor: int) : string =
        let outsideCount =
            if isNull src || src.Length = 0 then
                ...
                outside src |> Array.filter id |> Array.length
        ...
        + string outsideCount
        + " len="
        + string (if isNull src then 0 else src.Length)

    /// 名前の数え方を突き合わせる口。表は引数。ここで表を作るな。
    /// 配列で受ける。list は焼くと連結リストになり、素の配列を渡せなくなる。
    let describeTags (labels: (string * string * int * string)[]) (attrName: string) (src: string) : string =
