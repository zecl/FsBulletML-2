...
///     (name (@ (attr "値") ...) "本文" 子...)
///
/// **XML より楽。** 閉じ札も自己閉じも無く、コメントも宣言も CDATA も無い
/// （sxml のパーサがどれも読まない）。括弧と引用符だけ数えれば足りる。
///
/// **パーサは使わない。** 理由は XML 側と同じ —— 打っている途中の本文は
/// 必ず壊れている。`Sxml.parse`（FParsec）は落ちた位置を返してくれるので
...
module SxmlScan =
  ...
  /// （XML 側は 1 つ なので付かない）—— 突き合わせの表が指す名前が、
  /// **中身と関係ないところで表記ごとに割れる**
  type Form =
    { Hit: TagHit
      /// `(@` の `(` の位置。属性ブロックが無ければ -1
  ...
  /// 括弧を頭から全部 拾う。**木は組まない**が、`Start` / `Stop` が
  /// 子を丸ごと覆うので、入れ子は位置から読める（いちばん内側 = 前順で最後）。
  ///
  /// **閉じ引用符が無い属性も捨てない。** XML 側は捨てているが、あちらは
  /// カーソルの居場所を別の走査（`inTag`）で出しているので困らない。
  /// こちらは 1 本 で出すので、捨てると `(type "` まで打った時点で
  /// **属性値の候補が静かに出なくなる。**
  ///
  /// 捨てないぶん、閉じていない値は「本文の末尾まで」になる。参照を数える側が
  /// それを拾うことは無い —— 閉じ引用符が無ければ `Sxml.parse` が**位置つきで**
  /// 落ち、`References.explain` は位置が在る層をそのまま返して `missing` を
  /// 呼ばないため（`Parser.Tests/SxmlReferenceScan.fs` が当てている）。
