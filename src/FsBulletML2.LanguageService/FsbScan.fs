...
module FsbScan =
    ...
    let rows (src: string) : Row list =
        ...
        while i <= n do
            ...
            while p < lineStop && src.[p] = ' ' do
                p <- p + 1

            let indent = p - lineStart
            let nameFrom = p

            while p < lineStop && isName src.[p] do
                p <- p + 1
    ...
    let contextAt (src: string) (offset: int) : Context =
        ...
        | Some r when cursor > r.Hit.NameStop ->
            match
                r.Hit.Attrs
                |> List.tryFind (fun a -> cursor >= a.ValueStart && cursor <= a.ValueStop)
            with
            | Some a -> InAttrValue(r.Hit.TagName, a.AttrName)
            | None ->
                // 本文の中は、その要素の #PCDATA
                if r.BodyStart >= 0 && cursor >= r.BodyStart && cursor <= r.BodyStop then
                    InContent(Some r.Hit.TagName)
                else
                    InStartTag r.Hit.TagName
        | _ ->
            let indent =
                match here with
                | Some r -> r.Indent
    ...
    let tokenAt (src: string) (offset: int) : Token =
        if src.Length = 0 then
            ...
            | Some r ->
                ...
                if p >= h.NameStart && p < h.NameStop then
                    Element h.TagName
                else
    ...
    let texts (src: string) (names: string list) : TextHit list =
        tags src
        |> List.choose (fun t ->
            if not (List.contains t.TagName names) then
                None
            else
                ...
                while found.IsNone && i <= stop do
                    ...
                    if inQuote then
                        ...
                        while k < src.Length && (src.[k] = ' ' || src.[k] = '\t') do
                            k <- k + 1
