...
///         name:"本文"
///
/// **入れ子は行頭の空白で決まる。** 閉じ札が無いので、
/// `Start` / `Stop` が子を覆うことも無い（XML と sxml は覆う）。
/// **覆わなくて困る側は無い** —— 参照を数える `References.missing` が読むのは
/// `TagName` と `Attrs` と `Closing` だけで、入れ子を見ていない（v1.1 で確かめた）。
///
/// **区切りは空白 1 文字 の重み。** `Offside.fs` の字下げは `pchar ' '` だけで、
/// **タブを字下げに使えない。** ここも空白だけ数える。
///
/// 広く取ると、**パーサが捨てる行に補完だけが出る** ——
/// `Offside.parse` は `eof` を要求していないので、タブの行はエラーにならず
/// **そこで読むのをやめて、残りを黙って捨てる**（v1.1 の頭で測った）。
///
/// **パーサは使わない。** 理由は XML / sxml と同じ —— 打っている途中の本文は
/// 必ず壊れている。`Offside.parse`（FParsec）は落ちた位置を返してくれるので
/// 読む段ではそちらを使うが、補完と hover は壊れた本文でも止まらない側が要る。
///
...
module FsbScan =
  ...
  /// 要るので、そこだけこちらが持つ。
  ///
  /// **module の中に置いてある。** 名前空間の側へ出すと、この 1 本 が
  /// 「型 + module」の 2 つ になって Fable の export 名に接頭辞が付く
  /// —— 突き合わせの表が指す名前が、**中身と関係ないところで表記ごとに割れる**
  /// （理由は `SxmlScan.fs` の `Form` と同じ）
  type Row =
    { Hit: TagHit
  ...
  ///
  /// あちらは `:` も `-` も `.` も通すが、fsb では `:` が**本文の区切り**で、
