...
module XmlScan =
    ...
    let tags (src: string) : TagHit list =
        ...
        let skipUntil (s: string) =
            while i < src.Length
                  && not (i + s.Length <= src.Length && src.Substring(i, s.Length) = s) do
                advance ()
        ...
        while i < src.Length do
            if src.[i] <> '<' then
                advance ()
            elif i + 4 <= src.Length && src.Substring(i, 4) = "<!--" then
                skipUntil "-->"
            elif i + 9 <= src.Length && src.Substring(i, 9) = "<![CDATA[" then
                skipUntil "]]>"
            elif i + 2 <= src.Length && (src.[i + 1] = '!' || src.[i + 1] = '?') then
                skipUntil ">"
            else
                let start = i
                advance () // '<'
                ...
                while i < src.Length && Scan.isNameChar src.[i] do
                    advance ()
                ...
                while stop < 0 && i < src.Length do
                    if src.[i] = '>' then
                        ...
                        stop <- i
                        advance ()
                    elif Char.IsWhiteSpace src.[i] || src.[i] = '/' then
                        advance ()
                        ...
                        while i < src.Length && Char.IsWhiteSpace src.[i] do
                            advance ()
                        ...
                        if i < src.Length && src.[i] = '=' then
                            ...
                            if i < src.Length && (src.[i] = '"' || src.[i] = '\'') then
                                ...
                                while i < src.Length && src.[i] <> quote do
                                    advance ()
                ...
                if name <> "" then
                    ...
                    if not closing && not selfClosing then
                        depth <- depth + 1
    ...
    let private inTag (text: string) : Context option =
        ...
        then
            ...
            if k >= text.Length then
                ...
                if quote <> '\000' then
                    Some(InAttrValue(name, lastAttr))
                else
                    Some(InStartTag name)
    ...
    let tokenAt (src: string) (offset: int) : Token =
        ...
        if src.Length = 0 then
            ...
            | None -> Nothing
            | Some t ->
                if p >= t.NameStart && p < t.NameStop then
                    Element t.TagName
                else
    ...
    let describe (src: string) (cursor: int) : string =
        Scan.describe
            (tags src)
            (contextAt src cursor)
            (tokenAt src cursor)
            (Scan.nameLenBefore src cursor)
            (Scan.exprLenBefore src cursor)
    ...
    let texts (src: string) (names: string list) : TextHit list =
        tags src
        |> List.choose (fun t ->
            if t.Closing || t.SelfClosing || not (List.contains t.TagName names) then
                None
            else
                let from = t.Stop + 1

                if from >= src.Length then
                    None
                else
                    let mutable j = from

                    while j < src.Length && src.[j] <> '<' do
                        j <- j + 1
